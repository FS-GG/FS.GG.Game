...
AC-04 | supported | checkpoints=Origin.ProductionJourney | terminal=n/a | route=opaque JourneyReceipt + executable IProductionJourneyProof | reason=Playable and text artifacts cannot create accepted production provenance
AC-05 | supported | checkpoints=Proofs.loadJourneyProofs | terminal=result=pass | route=in-process receipt + TRX test identity | reason=JSON, caller keys, and hand-authored tokens are not provenance inputs
...
AC-08 | supported | checkpoints=Composition.script | terminal=Screen=Won | route=FS.GG.Game.Reference application boot/map/update/tick/effect | reason=tests import a separate product composition root
...
AC-13 | supported | checkpoints=ReferenceProof production gate | terminal=Screen=Won | route=FS.GG.Game.Reference composition | reason=constructed Pong states are no longer called complete E2E proof
AC-14 | supported | checkpoints=CLI + harness failure tests | terminal=pass required | route=proof assembly/critic/Unbound/exhaustion/origin | reason=text/key self-attestation, missing, simulation, critic-veto, unbound, non-terminal cases fail closed
