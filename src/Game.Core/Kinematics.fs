...
module Kinematics =
    ...
    let contact (moving: Rect) (shape: KinematicShape) : Contact option =
        ...
        | KinematicShape.Circle value ->
            ...
            |> Option.map (fun found ->
                { Normal = { X = -found.Normal.X; Y = -found.Normal.Y }
                  Depth = found.Depth })
    ...
    let candidates (cellSize: float) (motion: KinematicMotion) (colliders: KinematicCollider list) : KinematicCollider list =
        colliders
    ...
    let private translate (displacement: Point) (rect: Rect) : Rect =
        { rect with X = rect.X + displacement.X; Y = rect.Y + displacement.Y }
    ...
    let private expandedForMover (moving: Rect) (shape: KinematicShape) : KinematicShape =
        ...
        | KinematicShape.Circle value ->
            KinematicShape.Circle { value with Radius = value.Radius + sqrt (halfWidth * halfWidth + halfHeight * halfHeight) }
    ...
    let private sweep (moving: Rect) (displacement: Point) (shape: KinematicShape) : RayHit option =
        ...
        let p1 = { X = p0.X + displacement.X; Y = p0.Y + displacement.Y }
        segmentHit p0 p1 (expandedForMover moving shape)

    let private hit (isTrigger: bool) (collider: KinematicCollider) (overlap: Contact option) (swept: RayHit option) : KinematicHit =
        let contactValue =
            match overlap, swept with
            | Some value, _ -> value
            | None, Some value ->
                { Normal = { X = -value.Normal.X; Y = -value.Normal.Y }
                  Depth = 0.0 }
            | None, None -> { Normal = { X = 0.0; Y = 0.0 }; Depth = 0.0 }
    ...
    let private bounce (restitution: float) (displacement: Point) (normal: Point) : Point =
        let e = if finite restitution then max 0.0 (min 1.0 restitution) else 0.0
    ...
    let advance (cellSize: float) (motion: KinematicMotion) (colliders: KinematicCollider list) : KinematicResult =
        ...
        let observations =
            ...
            |> List.choose (fun collider ->
                let overlap = contact endBounds collider.Shape |> Option.orElseWith (fun () -> contact motion.Bounds collider.Shape)
                let swept = sweep motion.Bounds motion.Displacement collider.Shape
                if overlap.IsNone && swept.IsNone then None
                else
        ...
        let solid =
            ...
            |> List.sortBy (fun (collider, value) -> value.Sweep |> Option.map _.T |> Option.defaultValue -1.0, collider.Id)
            |> List.tryHead
        ...
        let finalBounds, remaining =
            ...
            | Some(collider, value) ->
                ...
                | Some swept ->
                    let travel = { X = motion.Displacement.X * swept.T; Y = motion.Displacement.Y * swept.T }
                    ...
                    let response =
                        ...
                        | KinematicResponse.Bounce restitution -> bounce restitution motion.Displacement value.Contact.Normal
                        | KinematicResponse.Trigger -> motion.Displacement
                    atHit, response
                | None ->
                    let separated = Resolution.pushOut (Geometry.center endBounds) value.Contact
                    let corrected = { endBounds with X = separated.X - endBounds.Width / 2.0; Y = separated.Y - endBounds.Height / 2.0 }
                    let response =
                        match collider.Response with
                        | KinematicResponse.Slide -> Resolution.slide motion.Displacement value.Contact.Normal
                        | KinematicResponse.Bounce restitution -> bounce restitution motion.Displacement value.Contact.Normal
