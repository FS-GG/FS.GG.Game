...
module SaveMigration =
    ...
    let private validateIdentity identity =
        if blank identity.EngineId then
            Some(SaveRefusal.InvalidIdentity "engine-id")
        elif blank identity.EngineVersion then
            Some(SaveRefusal.InvalidIdentity "engine-version")
        elif blank identity.ProfileId then
            Some(SaveRefusal.InvalidIdentity "profile-id")
        elif blank identity.SchemaId then
            Some(SaveRefusal.InvalidIdentity "schema-id")
        elif identity.SchemaVersion < 1 then
            Some(SaveRefusal.InvalidIdentity "schema-version")
        elif blank identity.ContentHash then
            Some(SaveRefusal.InvalidIdentity "content-hash")
        elif blank identity.AssetHash then
            Some(SaveRefusal.InvalidIdentity "asset-hash")
        else
            None
            ...
            Ok
                {
                    ...
                    Generation = 0UL
                    NextOperation = 0UL
                    Status = SaveMigrationStatus.Idle
                    LastCommitted = None
                    Pending = None
                }

    let private compatible expected actual =
        if expected.Family <> actual.Family then
            Some(SaveRefusal.FamilyMismatch(expected.Family, actual.Family))
        elif
            expected.EngineId <> actual.EngineId
            || expected.EngineVersion <> actual.EngineVersion
        then
            Some(
                SaveRefusal.EngineMismatch(
                    expected.EngineId,
        ...
        elif expected.ProfileId <> actual.ProfileId then
            Some(SaveRefusal.ProfileMismatch(expected.ProfileId, actual.ProfileId))
        elif expected.SchemaId <> actual.SchemaId then
            Some(SaveRefusal.SchemaMismatch(expected.SchemaId, actual.SchemaId))
        elif expected.ContentHash <> actual.ContentHash then
            Some(SaveRefusal.ContentHashMismatch(expected.ContentHash, actual.ContentHash))
        elif expected.AssetHash <> actual.AssetHash then
            Some(SaveRefusal.AssetHashMismatch(expected.AssetHash, actual.AssetHash))
        elif actual.SchemaVersion > expected.SchemaVersion then
            Some(SaveRefusal.NewerSchema(expected.SchemaVersion, actual.SchemaVersion))
        else
            None
    ...
    let private validatePlan steps =
        ...
        let rec loop prior remaining =
            ...
            | [] -> None
            | step :: tail when step.FromVersion < 1 || step.ToVersion <> step.FromVersion + 1 ->
                Some(SaveRefusal.InvalidMigrationPlan "steps must advance one positive version")
            | step :: _ when prior = Some step.FromVersion ->
                Some(SaveRefusal.InvalidMigrationPlan "duplicate or cyclic source version")
    ...
    let update hashPayload steps observation state =
        let refuse reason =
            state, [ SaveMigrationEffect.Refused reason ]
        ...
        let start envelope generation effects =
            ...
            | Some refusal ->
                { state with
                    ...
                    Status = SaveMigrationStatus.Failed refusal
                    Pending = None
                },
                effects @ [ SaveMigrationEffect.Refused refusal ]
            | None ->
                ...
                | None when hashPayload envelope.Payload <> envelope.PayloadHash ->
                    let refusal =
                        SaveRefusal.PayloadHashMismatch(envelope.PayloadHash, hashPayload envelope.Payload)
                ...
                | None when envelope.Identity.SchemaVersion = state.Expected.SchemaVersion ->
                    let accepted =
                        { envelope with
                            Identity = state.Expected
                        }

                    { state with
                        Generation = generation
                        Status = SaveMigrationStatus.Ready
                        LastCommitted = Some accepted
                        Pending = None
                    },
                    effects @ [ SaveMigrationEffect.Accepted accepted ]
                | None ->
                    ...
                    | None ->
                        match
                            steps
                            |> List.tryFind (fun step -> step.FromVersion = envelope.Identity.SchemaVersion)
                        with
                        | None ->
                            let refusal =
                                SaveRefusal.MissingMigration(
                                    envelope.Identity.SchemaVersion,
                                    envelope.Identity.SchemaVersion + 1
                                )
                        ...
                        | Some step ->
                            ...
                            | Error diagnostic ->
                                let refusal =
                                    SaveRefusal.MigrationFailed(step.FromVersion, step.ToVersion, diagnostic)
                            ...
                            | Ok payload ->
                                ...
                                { state with
                                    Generation = generation
                                    NextOperation = state.NextOperation + 1UL
                                    Status = SaveMigrationStatus.Migrating
                                    Pending = Some(id, migrated)
                                },
                                effects @ [ SaveMigrationEffect.PersistStep(id, migrated) ]
        ...
        | _, SaveMigrationObservation.Dispose ->
            { state with
                Status = SaveMigrationStatus.Disposed
                Pending = None
            },
            [ SaveMigrationEffect.Disposed ]
        | _, SaveMigrationObservation.Replace envelope ->
            if state.Generation = System.UInt64.MaxValue then
                refuse (SaveRefusal.InvalidMigrationPlan "generation exhausted")
            else
                let generation = state.Generation + 1UL in
                start envelope generation [ SaveMigrationEffect.GenerationInvalidated(state.Generation, generation) ]
        ...
        | _, SaveMigrationObservation.StepCommitted id ->
            ...
            | Some(expected, envelope) when expected = id ->
                let committed =
                    { state with
                        LastCommitted = Some envelope
                        Pending = None
                    }

                start envelope state.Generation []
                |> fun (next, effects) ->
                    { next with
                        LastCommitted = committed.LastCommitted
                    },
                    effects
        ...
        | _, SaveMigrationObservation.StepFailed(id, diagnostic) ->
            ...
            | Some(expected, _) when expected = id ->
                ...
                [ SaveMigrationEffect.Refused refusal ]
            | Some(expected, _) -> refuse (SaveRefusal.StaleOperation(expected, id))
            | None -> refuse SaveRefusal.Inactive
...
module Autosave =
    let initialize config committed =
        if config.DebounceMilliseconds = 0UL then
            Error(SaveRefusal.InvalidIdentity "debounce-milliseconds")
        else
            Ok
                {
                    Config = config
    ...
    let private dispatch state value =
        if state.NextOperation = System.UInt64.MaxValue then
            ...
            { state with
                ...
                Draft = None
                InFlight = Some(id, value)
                DueAtMilliseconds = None
            },
            [ AutosaveEffect.Persist(id, value) ]

    let update observation state =
        let refuse reason =
            state, [ AutosaveEffect.Refused reason ]

        match state.Status, observation with
        | AutosaveStatus.Disposed, AutosaveObservation.Dispose -> state, []
        | AutosaveStatus.Disposed, _ -> refuse SaveRefusal.Inactive
        | _, AutosaveObservation.Dispose ->
            { state with
                Status = AutosaveStatus.Disposed
                Draft = None
                InFlight = None
                DueAtMilliseconds = None
            },
            [ AutosaveEffect.Disposed ]
        | _, AutosaveObservation.Replace value ->
            if state.Generation = System.UInt64.MaxValue then
                ...
                [ AutosaveEffect.GenerationInvalidated(state.Generation, generation) ]
        | _, AutosaveObservation.Cancel ->
            { state with
                Status = AutosaveStatus.Cancelled
                Draft = None
                InFlight = None
                DueAtMilliseconds = None
            },
        ...
        | AutosaveStatus.Persisting, AutosaveObservation.Edit(now, value) ->
            let due =
                if System.UInt64.MaxValue - now < state.Config.DebounceMilliseconds then
                    System.UInt64.MaxValue
                else
                    now + state.Config.DebounceMilliseconds

            { state with
                Draft = Some value
                DueAtMilliseconds = Some due
            },
            []
        | _, AutosaveObservation.Edit(now, value) ->
            ...
            { state with
                Status = AutosaveStatus.Debouncing
                Draft = Some value
                DueAtMilliseconds = Some due
            },
            []
        | AutosaveStatus.Debouncing, AutosaveObservation.Tick now ->
            match state.DueAtMilliseconds, state.Draft with
            | Some due, Some value when now >= due -> dispatch state value
            | _ -> state, []
        | AutosaveStatus.Failed _, AutosaveObservation.Retry ->
            match state.Draft with
            | Some value -> dispatch state value
            | None -> refuse SaveRefusal.Inactive
        | AutosaveStatus.Persisting, AutosaveObservation.Persisted id ->
            match state.InFlight with
            | Some(expected, value) when expected = id ->
                let status =
                    if state.Draft.IsSome then
                ...
                { state with
                    Status = status
                    LastCommitted = Some value
                    InFlight = None
                },
                [ AutosaveEffect.Committed(id, value) ]
            | Some(expected, _) -> refuse (SaveRefusal.StaleOperation(expected, id))
            | None -> refuse SaveRefusal.Inactive
        | AutosaveStatus.Persisting, AutosaveObservation.PersistFailed(id, diagnostic) ->
            ...
            | Some(expected, value) when expected = id ->
                ...
                { state with
                    Status = AutosaveStatus.Failed diagnostic
                    Draft = draft
                    InFlight = None
                },
                [ AutosaveEffect.Refused(SaveRefusal.OperationFailed diagnostic) ]
            | Some(expected, _) -> refuse (SaveRefusal.StaleOperation(expected, id))
            | None -> refuse SaveRefusal.Inactive
        | _, AutosaveObservation.Persisted id ->
            refuse (
                SaveRefusal.StaleOperation(
                    {
                        Generation = state.Generation
                        Operation = state.NextOperation
                    },
        ...
        | _, AutosaveObservation.PersistFailed(id, _) ->
            refuse (
                SaveRefusal.StaleOperation(
                    {
                        Generation = state.Generation
