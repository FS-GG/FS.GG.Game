...
[<RequireQualifiedAccess>]
module Replay =
    let private present value = not (System.String.IsNullOrWhiteSpace value)

    let validate recording =
        let issues = ResizeArray<ReplayIssue>()
        if recording.FormatVersion <> 1 then issues.Add(ReplayIssue.UnsupportedFormatVersion recording.FormatVersion)
        let snapshotIssues = SessionEnvelope.validateSnapshot recording.InitialSnapshot
        if not snapshotIssues.IsEmpty then issues.Add(ReplayIssue.InvalidSession snapshotIssues)
        ...
        let compatibilityIssues = SessionCompatibility.compare recording.Compatibility recording.InitialSnapshot.Compatibility
        if not compatibilityIssues.IsEmpty then issues.Add(ReplayIssue.IncompatibleSnapshot compatibilityIssues)
        if not (present recording.InitialStateDigest) then issues.Add(ReplayIssue.MissingStateDigest None)

        let mutable expectedIndex = 0UL
        let mutable lastSequence = None
        for event in recording.Events do
            if event.Index <> expectedIndex then issues.Add(ReplayIssue.InvalidEventIndex(expectedIndex, event.Index))
            if not (present event.StateDigest) then issues.Add(ReplayIssue.MissingStateDigest(Some event.Index))
            ...
            | ReplayEventKind.Input input ->
                ...
                if not inputIssues.IsEmpty then issues.Add(ReplayIssue.InvalidSession inputIssues)
        ...
        for checkpoint in recording.Checkpoints do
            ...
            if not checkpointIssues.IsEmpty then issues.Add(ReplayIssue.InvalidSession checkpointIssues)
            ...
            let compatibilityIssues = SessionCompatibility.compare recording.Compatibility checkpoint.Snapshot.Compatibility
            if not compatibilityIssues.IsEmpty then issues.Add(ReplayIssue.IncompatibleSnapshot compatibilityIssues)
    ...
    let seek contract stateDigest shouldCancel targetEventCount recording =
        ...
        let eventCount = uint64 recording.Events.Length
        if targetEventCount > eventCount then Error(issues @ [ ReplayIssue.TargetBeyondRecording(targetEventCount, eventCount) ])
        elif not issues.IsEmpty then Error issues
        else
                ...
                let rec execute state remaining =
                    ...
                    | event::tail ->
                        let result =
                            ...
                            | ReplayEventKind.Advance steps -> contract.Advance { SessionId=recording.SessionId;StepCount=steps } state
                        match result with
                        ...
                        | Ok next ->
                            ...
                            if actual <> event.StateDigest then
                                Ok(ReplayRunOutcome.Diverged
                                    { EventIndex=event.Index;ExpectedDigest=event.StateDigest;ActualDigest=actual })
                            else execute next tail
    ...
    let create (initialSnapshot: SessionSnapshot<'snapshot>) initialStateDigest =
        let recording: ReplayRecording<'input, 'snapshot> =
            { FormatVersion=1
              ...
              Events=[]
              Checkpoints=[] }
        match Replay.validate recording with [] -> Ok recording | issues -> Error issues

    let private append kind digest recording =
        let event = { Index=uint64 recording.Events.Length;Kind=kind;StateDigest=digest }
        let candidate = { recording with Events=recording.Events @ [ event ] }
        match Replay.validate candidate with [] -> Ok candidate | issues -> Error issues

    let appendInput input stateDigest recording = append (ReplayEventKind.Input input) stateDigest recording
    let appendAdvance stepCount stateDigest recording = append (ReplayEventKind.Advance stepCount) stateDigest recording

    let addCheckpoint snapshot stateDigest recording =
        let checkpoint = { NextEventIndex=uint64 recording.Events.Length;Snapshot=snapshot;StateDigest=stateDigest }
        let candidate = { recording with Checkpoints=recording.Checkpoints @ [ checkpoint ] }
        match Replay.validate candidate with [] -> Ok candidate | issues -> Error issues
...
module ReplayExport =
    ...
    let private compatibility value =
        [ string value.ContractVersion; value.EngineId; value.EngineVersion; value.ProfileId; value.SchemaId; string value.SchemaVersion ]
        ...
        | [] ->
            let encodeEvent event =
                let kind =
                    match event.Kind with
                    | ReplayEventKind.Input input ->
                        "I" + field input.SessionId + field input.InputId + field (string input.Sequence) + field (encodeInput input.Value)
