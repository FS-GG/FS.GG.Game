...
type SessionAdvance =
    { SessionId: string; StepCount: uint64 }
...
module SessionCompatibility =
    ...
    let validate value =
        [
            if value.ContractVersion <= 0 then
                SessionContractIssue.InvalidContractVersion value.ContractVersion
            if missing value.EngineId then
                SessionContractIssue.MissingEngineId
            if missing value.EngineVersion then
                SessionContractIssue.MissingEngineVersion
            if missing value.ProfileId then
                SessionContractIssue.MissingProfileId
            if missing value.SchemaId then
                SessionContractIssue.MissingSchemaId
            if value.SchemaVersion <= 0 then
                SessionContractIssue.InvalidSchemaVersion value.SchemaVersion
        ]

    let compare expected actual =
        [
            if expected.ContractVersion <> actual.ContractVersion then
                SessionCompatibilityIssue.ContractVersion(expected.ContractVersion, actual.ContractVersion)
            if expected.EngineId <> actual.EngineId then
                SessionCompatibilityIssue.EngineId(expected.EngineId, actual.EngineId)
            if expected.EngineVersion <> actual.EngineVersion then
                SessionCompatibilityIssue.EngineVersion(expected.EngineVersion, actual.EngineVersion)
            if expected.ProfileId <> actual.ProfileId then
                SessionCompatibilityIssue.ProfileId(expected.ProfileId, actual.ProfileId)
            if expected.SchemaId <> actual.SchemaId then
                SessionCompatibilityIssue.SchemaId(expected.SchemaId, actual.SchemaId)
            if expected.SchemaVersion <> actual.SchemaVersion then
                SessionCompatibilityIssue.SchemaVersion(expected.SchemaVersion, actual.SchemaVersion)
        ]
...
module SessionEnvelope =
    ...
    let validateInitialization (value: SessionInitialization<'configuration>) =
        ...
        @ SessionCompatibility.validate value.Compatibility

    let validateInput (value: SessionInput<'input>) =
        [
            if System.String.IsNullOrWhiteSpace value.SessionId then
                SessionContractIssue.MissingSessionId
            if System.String.IsNullOrWhiteSpace value.InputId then
                SessionContractIssue.MissingInputId
        ]
