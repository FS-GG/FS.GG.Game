...
module Workload =
    ...
    let private maximum a b =
        {
            ...
            InterpolatedMovingEntityCount = max a.InterpolatedMovingEntityCount b.InterpolatedMovingEntityCount
    ...
    let evaluateCost budget samples =
        ...
        let incompleteInterpolation =
            samples
            |> List.tryFind (fun sample -> sample.InterpolatedMovingEntityCount < sample.MovingEntityCount)
    ...
    let private step (playable: Playable<'world, 'key>) (keys: 'key list) (world: 'world) : 'world =
        let commands = keys |> List.choose (Playable.resolve playable)

        let applied =
            commands |> List.fold (fun state command -> playable.Apply command state) world
    ...
    let renderArtifact (hostProfile: string) (observations: WorkloadObservation list) =
        ...
        builder.Append(",\"measurementCapability\":\"bounded-headless-update-and-scene-route\"")
        |> ignore

        builder.Append(",\"notAuthoritativeFor\":\"live-compositor,swapchain,vblank,vsync\"")
        |> ignore

        builder.Append(",\"hostProfile\":").Append(quoted hostProfile) |> ignore

        builder.Append(",\"warmupSamplePolicy\":\"per-workload; monotonic Stopwatch; warmup excluded\"")
        |> ignore
        ...
        |> List.iteri (fun index observation ->
            ...
            builder.Append(",\"definition\":").Append(quoted observation.Definition)
            |> ignore

            builder
                .Append(",\"class\":")
                .Append(quoted (classToken observation.Class))
                .Append(",\"workloadKind\":")
                .Append(quoted (kindToken observation.Kind))
            |> ignore
            ...
            builder.Append(",\"perceptionWorkUnits\":").Append(cost.PerceptionWorkUnits)
            |> ignore

            builder.Append(",\"pathfindingWorkUnits\":").Append(cost.PathfindingWorkUnits)
            |> ignore

            builder.Append(",\"sceneNodesByLayer\":{\"product-scene\":").Append(cost.SceneNodeCount).Append('}')
            |> ignore

            builder.Append(",\"passed\":").Append(if observation.Verdict.Passed then "true" else "false")
            |> ignore
