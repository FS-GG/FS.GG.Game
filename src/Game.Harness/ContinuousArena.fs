...
module ContinuousArena =
    ...
    let init (config: ContinuousArenaConfig) : ContinuousArenaWorld =
        { Player = config.PlayerStart
          ...
          Score = 0
          Collected = Set.empty
          Outcome = if config.InitialHealth > 0 then ContinuousArenaOutcome.Running else ContinuousArenaOutcome.Lost }

    let apply (config: ContinuousArenaConfig) (command: Command) (world: ContinuousArenaWorld) : ContinuousArenaWorld =
        if command = Command.Fire && world.Outcome <> ContinuousArenaOutcome.Running then init config
        elif world.Outcome <> ContinuousArenaOutcome.Running then world
        else
            let speed = if finite config.Speed then max 0.0 config.Speed else 0.0
            match command with
            | Command.MoveNorth -> { world with Velocity = { X = 0.0; Y = -speed } }
            | Command.MoveSouth -> { world with Velocity = { X = 0.0; Y = speed } }
            | Command.MoveWest -> { world with Velocity = { X = -speed; Y = 0.0 } }
            | Command.MoveEast -> { world with Velocity = { X = speed; Y = 0.0 } }
            | Command.Fire
            | Command.Pause -> world
    ...
    let step (config: ContinuousArenaConfig) (world: ContinuousArenaWorld) (dt: float) : ContinuousArenaWorld =
        if world.Outcome <> ContinuousArenaOutcome.Running || not (finite dt) || dt <= 0.0 then world
        else
            let itemByCollider = config.Items |> List.map (fun item -> "item:" + item.Id, item) |> Map.ofList
            let colliders = config.Obstacles @ (config.Items |> List.map itemCollider)
            let displacement: Point = { X = world.Velocity.X * dt; Y = world.Velocity.Y * dt }
            let result =
                Kinematics.advance config.CollisionCellSize { Bounds = world.Player; Displacement = displacement } colliders
            let hitItems =
                result.Hits
            ...
            let outcome =
                if health <= 0 then ContinuousArenaOutcome.Lost
                elif config.WinningScore > 0 && score >= config.WinningScore then ContinuousArenaOutcome.Won
                else ContinuousArenaOutcome.Running
            let velocity = { X = result.Displacement.X / dt; Y = result.Displacement.Y / dt }
    ...
    let playable (config: ContinuousArenaConfig) (keymap: Map<'key, Command>) (dt: float) : Playable<ContinuousArenaWorld, 'key> when 'key: comparison =
        { Init = init config
          Keymap = keymap
          Apply = apply config
          Step = step config
