...
- **`Journey`** — the stronger boot-to-outcome route. A `ProductionJourney` starts from a product's
  ...
  `emit-evidence` serializes the validated receipt as a versioned `journeyReceipt` map and generates
  a JUnit report from that same in-memory execution. The shared execution id and receipt-binding
  digest mean a caller-supplied stale or fabricated TRX cannot stand in for the journey run.
