...
let generate (reportSource: string) (proofs: Map<string, ValidatedJourneyProof>) : GeneratedReport =
...
let renderYaml (indent: string) (bound: BoundReceipt) : string list =
    ...
    [
        ...
        + yamlString (JourneyReceipt.compositionAuthority receipt)
        indent
        + "origin: "
        + (match JourneyReceipt.origin receipt with
           | Origin.ProductionJourney -> "production-journey"
        ...
        indent
        + "initialFingerprint: "
        + digest (JourneyReceipt.initialFingerprintDigest receipt)
        indent
        + "terminalFingerprint: "
        + digest (JourneyReceipt.terminalFingerprintDigest receipt)
        indent + "terminalPredicate:"
        indent
        + "  identity: "
        + yamlString (JourneyReceipt.terminalPredicateIdentity receipt)
        indent
        + "  reached: "
        + ((JourneyReceipt.terminalPredicateReached receipt).ToString().ToLowerInvariant())
