            ...
            |> Array.fold
                (fun state implementation ->
                    ...
                    | Ok proofs ->
                        ...
                        | null ->
                            Error(sprintf "cannot instantiate production journey proof %s" implementation.FullName)
                        | instance ->
                            ...
                            match proof with
                            | :? IProductionJourneyProofV1 as proofV1 when
                                String.IsNullOrWhiteSpace proofV1.CompositionAuthority
                                ...
                                ->
                                Error(
                                    sprintf "%s declares an empty schema-v1 journey identity" implementation.FullName
                                )
                            | :? IProductionJourneyProofV1 as proofV1 when
                                proofV1.CompositionAuthority <> JourneyReceipt.compositionAuthority receipt
                                ...
                                Error(
                                    sprintf
                                        "%s returned a receipt with mismatched schema-v1 journey identities"
                                        implementation.FullName
                                )
                            | :? IProductionJourneyProofV1 as proofV1 when
                                proofV1.CompositionAuthority <> expectedAuthority
                                ->
                                Error(
                                    sprintf
                            ...
                            | :? IProductionJourneyProofV1 ->
                                if String.IsNullOrWhiteSpace proof.TestId || proof.TestId <> testId then
                                    Error(
                                        sprintf
                                            "%s returned a receipt for mismatched test identity '%s'"
                                            implementation.FullName
                                            testId
                                    )
                                    ...
                                    String.IsNullOrWhiteSpace(JourneyReceipt.runnerIdentity receipt)
                                    || String.IsNullOrWhiteSpace(JourneyReceipt.runnerVersion receipt)
                                then
                                    ...
                                    Error(
                                        sprintf
                                            "production journey proof %s did not reach its terminal predicate"
                                            testId
                                    )
                                    ...
                                    Error(
                                        sprintf "production journey proof %s violates its declared step bound" testId
                                    )
                                elif
                                    [
                                ...
                                then
                                    Error(
                                        sprintf "production journey proof %s has an incomplete digest binding" testId
                                    )
...
let loadJourneyReceipts (assemblyPath: string) : Result<Map<string, ValidatedJourneyProof>, string> =
    Error(
        sprintf "cannot load production journey proofs from %s without an explicit producer authority; " assemblyPath
