    ...
    testList
        ...
        [
            ...
            <| fun _ ->
                ...
                Expect.isTrue
                    (LawReport.allPassed report)
                    (sprintf "all laws must pass; failures: %A" (LawReport.failures report))
            ...
            <| fun _ ->
                let run =
                    Driver.runBot playable observeLeft chaseBot 7UL 20 Driver.identityFingerprint
            ...
            <| fun _ ->
                let dir =
                    Path.Combine(Path.GetTempPath(), sprintf "playtest-golden-%d" (abs (hash scripts)))
