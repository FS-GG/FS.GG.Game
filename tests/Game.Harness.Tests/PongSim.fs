...
let applySeat (seat: Seat) (cmd: Command) (w: Pong) : Pong =
    ...
    | Seat.A ->
        { w with
            LeftY = clampPaddle (w.LeftY + dir)
        }
    | Seat.B ->
        { w with
            RightY = clampPaddle (w.RightY + dir)
        }
...
let chaseBot: Bot<struct (int * int)> =
    {
        Decide =
            fun (struct (ballY, top)) rng ->
                ...
                if ballY < centre then
                    struct ([ Command.MoveNorth ], rng)
                elif ballY > centre then
                    struct ([ Command.MoveSouth ], rng)
                else
                    let struct (up, rng') = Rng.nextBool rng
...
let sitterBot: Bot<struct (int * int)> =
    {
