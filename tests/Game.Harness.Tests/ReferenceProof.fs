...
let private detScript: string list list =
    [ [ "w" ]; [ "w" ]; [ "s" ]; []; [ "s" ]; [ "w" ] ]
    ...
    testList
        ...
        [
            ...
            <| fun _ ->
                ...
                let run =
                    Driver.runBot playable observeLeft chaseBot 7UL 120 Driver.identityFingerprint
            ...
            <| fun _ ->
                ...
                Expect.equal
                    (Playable.resolve playable "esc")
                    (Some Command.Pause)
                    "'esc' is bound to Pause (a no-move command), not silently unbound"
            ...
            <| fun _ ->
                ...
                Expect.isTrue
                    (scoringSteps
                     |> List.forall (fun (_, cur) -> cur.BallX = W / 2 && cur.BallY = H / 2))
                ...
                Expect.isGreaterThan
                    ...
                    1
                    "across seeds the launch direction varies — the seed is load-bearing"
            ...
            <| fun _ ->
                ...
                Expect.notEqual
                    (List.head forward)
                    (List.last forward)
                    "the outcome sequence is non-palindromic, so order is actually under test"

                let reversed =
                    Matrix.runMatrix matchSetup outcome (List.rev matches) |> List.map snd
            ...
            testCase
                "gate: the obligation chain is green — every gameplay-FR proof is a non-synthetic InputDriven trace"
            <| fun _ ->
                ...
                for (id, trace) in gameplayProofs do
                    ...
                    Expect.equal
                        (Trace.origin trace)
                        Origin.InputDriven
                        (sprintf "%s is satisfied by input-driven evidence" id)
            ...
            <| fun _ ->
                ...
                Expect.equal
                    (Trace.origin journey.Trace)
                    Origin.ProductionJourney
                    "the stronger evidence level is distinct"

                Expect.equal
                    (JourneyReceipt.result journey.Receipt)
                    JourneyResult.Passed
                    "boot-to-outcome reaches its terminal predicate"

                Expect.equal
                    (JourneyReceipt.testId journey.Receipt)
                    "GP-JOURNEY-001"
                    "receipt binds the scenario to its test identity"
            ...
            <| fun _ ->
                ...
                Expect.isTrue
                    (manifest |> List.forall (fun f -> f.Facet = "gameplay"))
                    "every FR carries the gameplay classifier facet"
                ...
                Expect.isTrue
                    (manifest
                     |> List.forall (fun f -> f.CoversAc |> List.forall (fun ac -> ac >= 1 && ac <= 19)))
