        ...
        [
            ...
            <| fun _ ->
                ...
                | PropertyResult.Falsified c ->
                    Expect.equal
                        c.Script
                        (List.replicate 4 [ Command.MoveNorth ])
                        "shrunk to exactly four MoveNorth frames"
                    ...
                    let final =
                        Driver.runCommands playable Driver.identityFingerprint c.Script
                        |> Trace.frames
                        |> List.last
            ...
            <| fun _ ->
                ...
                let cfg =
                    { Properties.defaultConfig with
                        Moves = Some [ [ Command.MoveSouth ] ]
                        Runs = 200
                    }
            ...
            <| fun _ ->
                ...
                let prop (indices: int list) =
                    let script =
                        indices
                        |> List.map (fun i -> alphabet.[((i % alphabet.Length) + alphabet.Length) % alphabet.Length])
