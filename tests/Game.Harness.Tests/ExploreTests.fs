    ...
    testList
        ...
        [
            ...
            <| fun _ ->
                ...
                Expect.equal
                    (Explore.findScript playable pongFp deflection 300)
                    FindResult.NotFound
                    "the degenerate fixture cannot deflect off the left paddle"

                let frontier = Explore.reachable playable pongFp 300

                let ballEverOnLeftHalf =
                    frontier.States |> Set.exists (fun (bx, _, _, _, _) -> bx < W / 2)

                Expect.isFalse
                    ballEverOnLeftHalf
                    "no reachable state has the ball on the left half — the degenerate frontier"
            ...
            <| fun _ ->
                ...
                | FindResult.Found w ->
                    ...
                    Expect.isTrue
                        (w.Script |> List.forall (fun f -> f = [ Command.MoveNorth ]))
                        "only the restricted move was explored"
            ...
            <| fun _ ->
                ...
                Expect.equal
                    (List.length moves)
                    (1
                     + (playable.Keymap |> Map.toList |> List.map snd |> List.distinct |> List.length))
                    "empty + one per distinct command"
        ]
