...
let tests =
    testList
        "ContinuousArena"
        [
            testCase "real command route collects, wins and restarts"
            <| fun _ ->
                let pickup =
                    {
                        Id = "gem"
                        Bounds = rect 2.0 0.0 0.5 1.0
                        Kind = ContinuousArenaItemKind.Collectible 1
                    }

                let arena = config [ pickup ] [] 1 3

                let playable =
                    ContinuousArena.playable arena (Map.ofList [ 'd', Command.MoveEast ]) 1.0
                ...
                let restarted =
                    ContinuousArena.apply
                        arena
                        Command.Fire
                        { ContinuousArena.init arena with
                            Outcome = ContinuousArenaOutcome.Won
                        }

                Expect.equal restarted (ContinuousArena.init arena) "fire restarts terminal authority state"

            testCase "hazard reduces health and produces loss"
            <| fun _ ->
                let hazard =
                    {
                        Id = "hazard"
                        Bounds = rect 2.0 0.0 0.5 1.0
                        Kind = ContinuousArenaItemKind.Hazard 2
                    }

                let arena = config [ hazard ] [] 99 2

                let world =
                    ContinuousArena.init arena |> ContinuousArena.apply arena Command.MoveEast
            ...
            testCase "obstacle response prevents penetration"
            <| fun _ ->
                let wall =
                    {
                        Id = "wall"
            ...
            testCase "independent harness runs are structurally identical"
            <| fun _ ->
                let pickup =
                    {
                        Id = "gem"
                ...
                let playable =
                    ContinuousArena.playable arena (Map.ofList [ 'd', Command.MoveEast; 's', Command.MoveSouth ]) 0.5
