    ...
    testList
        ...
        [
            ...
            <| fun _ ->
                ...
                let step () =
                    let struct (c, _) = bot.Decide (struct (0, 0)) (Rng.ofSeed 1UL) in c
            ...
            <| fun _ ->
                let draw rng =
                    let struct (b, rng') = Rng.nextBool rng in
                    struct ([ (if b then Command.Fire else Command.Pause) ], rng')
            ...
            <| fun _ ->
                ...
                let decide v =
                    let struct (c, _) = bot.Decide v (Rng.ofSeed 0UL) in c
            ...
            <| fun _ ->
                let score _ (m: Command) =
                    if m = Command.MoveSouth then 1.0 else 0.0
                ...
                let tie =
                    Bots.greedyToward (fun _ _ -> 0.0) [ Command.MoveNorth; Command.MoveSouth ]

                let struct (ct, _) = tie.Decide (struct (0, 0)) (Rng.ofSeed 0UL)
                Expect.equal ct [ Command.MoveNorth ] "a tie resolves to the first move"

                let struct (ce, _) =
                    (Bots.greedyToward score []).Decide (struct (0, 0)) (Rng.ofSeed 0UL)
            ...
            <| fun _ ->
                ...
                let withSitterBot =
                    Matrix.runMatrix matchSetup outcome (matchesWith sitterBot) |> List.map snd

                let withBotsSitter =
                    Matrix.runMatrix matchSetup outcome (matchesWith Bots.sitter) |> List.map snd
            ...
            <| fun _ ->
                ...
                let chase =
                    Bots.chase
                        ((fun (struct (t, _)) -> t), (fun (struct (_, s)) -> s))
                        Command.MoveWest
                        Command.MoveEast
                ...
                let struct (cs, _) =
                    (Bots.sitter: Bot<struct (int * int)>).Decide (struct (2, 5)) (Rng.ofSeed 0UL)
