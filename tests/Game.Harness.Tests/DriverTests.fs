    ...
    testList
        ...
        [
            ...
            <| fun _ ->
                ...
                let golden =
                    "9,5,1,-1,3,0,0|10,4,1,-1,2,0,0|11,3,1,-1,3,0,0|12,2,1,-1,3,0,0|13,1,1,-1,3,0,0|14,0,1,-1,2,0,0|8,6,1,-1,2,1,0"
            ...
            <| fun _ ->
                ...
                let t =
                    Driver.runCommands dtPlayable Driver.identityFingerprint (List.replicate n [])
                ...
                Expect.equal
                    last.Sum
                    (float n * dtPlayable.Dt)
                    "the accumulated dt is exactly steps * Dt (no variable/zero dt)"
            ...
            <| fun _ ->
                let run =
                    Driver.runBot playable observeLeft chaseBot 7UL 200 Driver.identityFingerprint
            ...
            <| fun _ ->
                let run =
                    Driver.runBot playable observeLeft chaseBot 1UL 0 Driver.identityFingerprint
