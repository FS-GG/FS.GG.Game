...
let runScript adapter script =
    Journey.runScriptWithIdentity Composition.inputIdentity Composition.terminalPredicateIdentity adapter script
...
let tests =
    testList
        ...
        [
            testCase
                "boots the shipped reference composition and proves start, pause, progression, sealed-door refusal, and terminal outcome"
            <| fun _ ->
                ...
                Expect.equal
                    (JourneyReceipt.origin run.Receipt)
                    Origin.ProductionJourney
                    "the receipt carries the production origin"

                Expect.equal
                    (JourneyReceipt.inputKind run.Receipt)
                    JourneyInputKind.FixedScript
                    "the fixed script route is explicit"
                ...
                Expect.equal
                    (JourneyReceipt.result run.Receipt)
                    JourneyResult.Passed
                    "the bounded journey reaches its terminal predicate"
                ...
                Expect.equal
                    frames.[8].Room
                    Composition.Room.Vault
                    "the sealed exit refuses interaction before room clear"
            ...
            <| fun _ ->
                ...
                | JourneyResult.Failed reason ->
                    Expect.stringContains
                        reason
                        "displayed action 'Interact' is unbound"
                        "failure names the production wiring gap"
            ...
            <| fun _ ->
                ...
                let report =
                    Journey.checkActionCoverage ActionCoverageFixtures.twoActionAdapter vocabulary committedScripts
            ...
            <| fun _ ->
                ...
                let report =
                    Journey.checkActionCoverage ActionCoverageFixtures.fullyWiredAdapter vocabulary committedScripts

                Expect.isTrue
                    (ActionCoverageReport.isClean report)
                    "full coverage over a rich-enough vocabulary stays green"

            testCase
                "checkActionCoverage flags a single-inhabitant vocabulary slot that empirical Unbound-tracking alone cannot see"
            <| fun _ ->
                ...
                let report =
                    Journey.checkActionCoverage ActionCoverageFixtures.unitMenuAdapter vocabulary committedScripts

                Expect.isFalse
                    (ActionCoverageReport.isClean report)
                    "a single-inhabitant action slot is a coverage gap on its own"
            ...
            <| fun _ ->
                ...
                mustThrow "terminal" (fun () ->
                    Journey.runScriptWithIdentity Composition.inputIdentity "" productionAdapter productionScript
                    |> ignore)

            testCase
                "definitionDigest is stable across receipts whose composition authority differs but every authored declaration is identical (FS.GG.Game#562)"
            <| fun _ ->
            ...
            <| fun _ ->
                ...
                let renamedTest =
                    runScript
                        { productionAdapter with
                            TestId = productionAdapter.TestId + "-renamed"
                        }
                        productionScript
            ...
            testCase
                "runScriptWithIdentity's composition-authority diagnostic names the likely cause, not only the mismatch (FS.GG.Game#565)"
            <| fun _ ->
                ...
                match thrown with
                | None -> failtest "a composition mixing product- and caller-authored functions must be rejected"
                | Some message ->
                    Expect.stringContains
                        message
