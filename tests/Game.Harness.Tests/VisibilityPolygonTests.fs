...
let tests =
    testList
        "Game.Harness VisibilityPolygon (022, FR-001..FR-006)"
        [
                ...
                Expect.isTrue
                    (poly
                     |> List.forall (fun v ->
                         let near =
                             {
                                 X = origin.X + (v.X - origin.X) * 0.99
                                 Y = origin.Y + (v.Y - origin.Y) * 0.99
                             }
            ...
            test "reproducible: identical inputs yield an identical polygon (FR-005)" {
                ...
                let walls =
                    [
                        ({ X = 2.0; Y = 6.0 }, { X = 6.0; Y = 6.0 })
                        ({ X = 7.0; Y = 2.0 }, { X = 7.0; Y = 8.0 })
                    ]
            ...
            test "totality: degenerate inputs do not throw (FR-006)" {
                ...
                Expect.isTrue
                    (VisibilityPolygon.polygon { X = -5.0; Y = -5.0 } room [] |> List.isEmpty |> not)
                    "origin outside bounds still returns a polygon"
                // empty segments (only bounds)
                Expect.isTrue
                    (VisibilityPolygon.polygon { X = 5.0; Y = 5.0 } room [] |> List.isEmpty |> not)
                    "empty segments ⇒ the bounds polygon"
                // zero-length segment
                ...
                Expect.isTrue
                    (VisibilityPolygon.polygon { X = 0.0; Y = 0.0 } room [] |> List.isEmpty |> not)
                    "origin on a corner does not throw"
            }
        ]
