...
let tests =
    testList
        "FS.GG.Game.Render Adapter (US1, FR-001)"
        [
            ...
            test "rect projects sim fields by identity" {
                let expected: FS.GG.UI.Scene.Rect =
                    {
                        X = 1.0
                        Y = 2.0
                        Width = 3.0
                ...
                Expect.equal
                    (FS.GG.Game.Render.Adapter.rect (simRect 1.0 2.0 3.0 4.0))
                    expected
                    "rect is identity on fields"
            }
            ...
            test "cellRect scales a discrete tile to its continuous square (integer->float seam)" {
                ...
                Expect.equal
                    (FS.GG.Game.Render.Adapter.cellRect 16.0 (cell 2 3))
                    expected
            ...
            test "cellCentre is the tile centre at cellSize" {
                ...
                Expect.equal
                    (FS.GG.Game.Render.Adapter.cellCentre 16.0 (cell 2 3))
                    expected
                    "((Col+0.5)*size, (Row+0.5)*size)"
            }
            ...
            test "drawRect emits the same filled rectangle as the Scene constructor" {
                ...
                Expect.equal
                    (FS.GG.Game.Render.Adapter.drawRect fill r)
                    expected
                    "drawRect = filledRectangle over projected rect"
            }

            test "drawCell emits the filled square for the tile" {
                let expected =
                    Scene.filledRectangle (FS.GG.Game.Render.Adapter.cellRect 16.0 (cell 1 1)) fill

                Expect.equal
                    (FS.GG.Game.Render.Adapter.drawCell 16.0 fill (cell 1 1))
                    expected
                    "drawCell = filledRectangle over cellRect"
            }
            ...
            test "drawCells groups the tiles in supplied order (deterministic)" {
                ...
                let expected =
                    Scene.group (cells |> List.map (FS.GG.Game.Render.Adapter.drawCell 8.0 fill))
            ...
            test "drawPath with fewer than two cells is empty (nothing to connect)" {
                ...
                Expect.equal
                    (FS.GG.Game.Render.Adapter.drawPath 16.0 paint [ cell 2 2 ])
                    Scene.empty
                    "single cell -> Scene.empty"
            }
            ...
            test "drawPath routes a Cell list through tile centres as a polyline" {
                ...
                let commands =
                    match centres with
                    | head :: tail -> Path.moveTo head.X head.Y :: (tail |> List.map (fun c -> Path.lineTo c.X c.Y))
                    | [] -> []
            ...
            test "drawPath projects a real A* route (astar result -> Scene)" {
                ...
                let route = Pathfinding.astar FourWay 1000 (fun _ -> true) (cell 0 0) (cell 3 3)
