...
let tests =
    testList "Kinematics" [
        testCase "narrow phase agrees for AABB circle and convex fixtures" <| fun _ ->
            ...
            let polygon = KinematicShape.Convex(Geometry.obbPolygon (point 2.0 1.0) (point 1.0 1.0) 0.25)
        ...
        testCase "broad phase contains every directly swept AABB" <| fun _ ->
            let colliders =
                [ for index in 0 .. 20 ->
                    { Id = string index
                      Shape = KinematicShape.AxisAlignedBox(rect (float index * 3.0) 0.0 1.0 1.0)
                      Response = KinematicResponse.Slide } ]
            let motion = { Bounds = rect 0.0 0.0 1.0 1.0; Displacement = point 30.0 0.0 }
            let candidateIds = Kinematics.candidates 4.0 motion colliders |> List.map _.Id |> Set.ofList
            let directIds =
                colliders
                |> List.choose (fun collider ->
                    match collider.Shape with
                    | KinematicShape.AxisAlignedBox target when Geometry.sweptIntersects motion.Bounds motion.Displacement target -> Some collider.Id
                    | _ -> None)
            for id in directIds do Expect.isTrue (candidateIds.Contains id) $"candidate {id}"

        testCase "segment casts preserve the shared query answers" <| fun _ ->
        ...
        testCase "swept motion cannot tunnel through a thin obstacle" <| fun _ ->
            let wall =
                { Id = "wall"
                  Shape = KinematicShape.AxisAlignedBox(rect 5.0 -2.0 0.1 4.0)
                  Response = KinematicResponse.Slide }
            let result =
                Kinematics.advance 2.0 { Bounds = rect 0.0 -0.5 1.0 1.0; Displacement = point 20.0 0.0 } [ wall ]
        ...
        testCase "triggers report without changing motion" <| fun _ ->
            let trigger =
                { Id = "pickup"
                  Shape = KinematicShape.AxisAlignedBox(rect 2.0 0.0 1.0 1.0)
                  Response = KinematicResponse.Trigger }
            let motion = { Bounds = rect 0.0 0.0 1.0 1.0; Displacement = point 4.0 0.0 }
        ...
        testCase "bounce reverses the normal component" <| fun _ ->
            let wall =
                { Id = "wall"
                  Shape = KinematicShape.AxisAlignedBox(rect 2.0 -2.0 1.0 4.0)
                  Response = KinematicResponse.Bounce 1.0 }
            let result = Kinematics.advance 2.0 { Bounds = rect 0.0 -0.5 1.0 1.0; Displacement = point 4.0 1.0 } [ wall ]
