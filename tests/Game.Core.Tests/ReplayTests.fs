...
module ReplayTests =
    ...
    let private identity =
        { ContractVersion=1;EngineId="counter";EngineVersion="1";ProfileId="portable/1";SchemaId="counter";SchemaVersion=1 }

    let private snapshot value =
        { SessionId="replay-1";Revision=uint64 value;Compatibility=identity;Value=value }
    ...
    let private contract : SessionContract<unit,int,int,int,int> =
        { Initialize=fun _ -> Ok 0
          ...
          Project=fun state -> { SessionId="replay-1";Revision=uint64 state;Value=state }
          Snapshot=snapshot
          Restore=fun value -> Ok value.Value }

    let private recording () =
        ReplayRecorder.create (snapshot 0) "0" |> Result.bind (ReplayRecorder.appendInput {SessionId="replay-1";InputId="counter.add";Sequence=1UL;Value=2} "2")
        |> Result.bind (ReplayRecorder.appendAdvance 3UL "5")
        |> Result.bind (ReplayRecorder.addCheckpoint (snapshot 5) "5")
        |> Result.bind (ReplayRecorder.appendInput {SessionId="replay-1";InputId="counter.add";Sequence=2UL;Value=4} "9")

    [<Tests>]
    let tests = testList "portable replay" [
        testCase "record, seek and checkpoint use the product contract" <| fun _ ->
        ...
        testCase "cancellation preserves a resumable accepted state" <| fun _ ->
            let value = recording () |> success
        ...
        testCase "the first divergent event is exact" <| fun _ ->
            let value = recording () |> success
            let mutated =
                { value with Checkpoints=[];Events=value.Events |> List.map (fun event -> if event.Index=1UL then {event with StateDigest="6"} else event) }
        ...
        testCase "malformed order and stale inputs are refused before restore" <| fun _ ->
            let value = recording () |> success
            let bad =
                { value with
                    Events = value.Events |> List.mapi (fun index event ->
                        if index=0 then { event with Index=4UL }
                        elif index=2 then { event with Kind=ReplayEventKind.Input {SessionId="replay-1";InputId="counter.add";Sequence=1UL;Value=1} }
                        else event) }
            ...
            Expect.contains issues (ReplayIssue.StaleInputSequence(2UL,1UL,1UL)) "input sequences stay monotonic"

        testCase "canonical export is unambiguous and repeatable" <| fun _ ->
            let value = recording () |> success
