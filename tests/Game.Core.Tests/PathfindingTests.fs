...
let tests =
    testList
        "Game.Core Pathfinding (US1, FR-001..FR-005)"
        [
            ...
            test "astar FourWay finds a straight corridor path, endpoints included" {
                ...
                let path =
                    Pathfinding.astar FourWay 1000 walk { Col = 0; Row = 0 } { Col = 3; Row = 0 }
            ...
            test "astar FourWay cannot use diagonals (longer Manhattan path)" {
                ...
                let path =
                    Pathfinding.astar FourWay 1000 walk { Col = 0; Row = 0 } { Col = 3; Row = 3 }
            ...
            test "astar EightWay refuses to cut the corner past a blocked orthogonal (D5)" {
                ...
                let path =
                    Pathfinding.astar EightWay 1000 walk { Col = 0; Row = 0 } { Col = 1; Row = 1 }
            ...
            test "astar routes around a wall with a single gap" {
                ...
                Expect.isFalse
                    (Option.get path |> List.exists (fun c -> blocked.Contains(c.Col, c.Row)))
            ...
            test "start = goal (walkable) yields the single-cell path" {
                ...
                Expect.equal
                    (Pathfinding.astar FourWay 100 walk { Col = 1; Row = 1 } { Col = 1; Row = 1 })
                    (Some [ { Col = 1; Row = 1 } ])
                    "[start]"

                Expect.equal
                    (Pathfinding.bfs EightWay 100 walk { Col = 1; Row = 1 } { Col = 1; Row = 1 })
                    (Some [ { Col = 1; Row = 1 } ])
                    "[start]"
            }
            ...
            test "a blocked start or goal yields None" {
                ...
                Expect.isNone
                    (Pathfinding.astar FourWay 100 walk { Col = 0; Row = 0 } { Col = 1; Row = 1 })
                    "blocked start"

                Expect.isNone
                    (Pathfinding.astar FourWay 100 walk { Col = 1; Row = 1 } { Col = 2; Row = 2 })
                    "blocked goal"
            }
            ...
            test "a walled-off goal yields None (bounded search terminates)" {
                ...
                let ring =
                    Set.ofList [ (3, 4); (4, 4); (5, 4); (3, 5); (5, 5); (3, 6); (4, 6); (5, 6) ]

                let walk = gridWalkable 8 8 ring

                let path =
                    Pathfinding.astar EightWay 5000 walk { Col = 0; Row = 0 } { Col = 4; Row = 5 }
            ...
            test "maxVisited <= 0 yields None" {
                ...
                Expect.isNone
                    (Pathfinding.bfs FourWay -1 walk { Col = 0; Row = 0 } { Col = 3; Row = 3 })
                    "negative budget"
            }
            ...
            test "an unreachable-within-budget goal terminates at the cap with None" {
                ...
                Expect.isNone
                    (Pathfinding.astar FourWay 3 walk { Col = 0; Row = 0 } { Col = 99; Row = 99 })
            ...
            test "bfs returns a hop-minimal path" {
                ...
                let path =
                    Pathfinding.bfs FourWay 1000 walk { Col = 0; Row = 0 } { Col = 4; Row = 0 }
            ...
            test "two equal-cost routes still yield one stable path (repeat-run byte-identity)" {
                ...
                Expect.isTrue
                    (a |> Option.get |> validPath walk { Col = 0; Row = 0 } { Col = 1; Row = 1 })
                    "and it is a valid path"
            }

            testCase "determinism: repeat calls are byte-identical over random grids (FsCheck)"
            <| fun () ->
                let prop (blockedRaw: (int * int) list) (sc: int) (sr: int) (gc: int) (gr: int) =
                    let blocked =
                        blockedRaw
                        |> List.map (fun (c, r) -> (((abs c) % 8), ((abs r) % 8)))
                        |> Set.ofList
                    ...
                    p1 = p2
                    && (match p1 with
                        | Some path -> validPath walk start goal path
                        | None -> true)

                Check.One(Config.QuickThrowOnFailure.WithMaxTest 500, prop)

            testCase "bfs determinism over random grids (FsCheck)"
            <| fun () ->
                let prop (blockedRaw: (int * int) list) (sc: int) (gc: int) =
                    let blocked =
                        blockedRaw
                        |> List.map (fun (c, r) -> (((abs c) % 6), ((abs r) % 6)))
                        |> Set.ofList
            ...
            test "distanceField charges the entered cell's cost (mud is dear to cross, not to stand on)" {
                ...
                Expect.equal
                    (Map.tryFind { Col = 1; Row = 0 } field)
                    (Some 10)
                    "standing on mud is free; leaving it onto clear ground costs 10"

                Expect.equal
                    (Map.tryFind { Col = 2; Row = 0 } field)
                    (Some 40)
                    "crossing mud costs 3×10, then 10 onto the goal"
            }
            ...
            test "distanceField takes the nearest of several goals" {
                ...
                let field =
                    Pathfinding.distanceField FourWay 1000 cost [ { Col = 0; Row = 0 }; { Col = 4; Row = 0 } ]
            ...
            test "distanceField totality: empty goals, all-impassable goals, and maxVisited <= 0" {
                ...
                Expect.equal
                    (Pathfinding.distanceField FourWay -1 cost [ { Col = 0; Row = 0 } ])
                    Map.empty
                    "negative budget"
                ...
                Expect.equal
                    (Pathfinding.distanceField FourWay 1000 blocked [ { Col = 0; Row = 0 } ])
                    Map.empty
                    "every goal impassable ⇒ empty"

                let mixed =
                    Pathfinding.distanceField FourWay 1000 blocked [ { Col = 0; Row = 0 }; { Col = 3; Row = 3 } ]
            ...
            test "distanceField honours maxVisited and returns only settled (final) values" {
                ...
                let field =
                    Pathfinding.distanceField FourWay 5 (fun _ -> 1) [ { Col = 0; Row = 0 } ]
                ...
                let full =
                    Pathfinding.distanceField FourWay 1000 (fun _ -> 1) [ { Col = 0; Row = 0 } ]
            ...
            test "distanceField cost accumulation cannot overflow into a negative distance" {
                ...
                Expect.isFalse
                    (Map.containsKey { Col = 2; Row = 0 } field)
                    "a step that would exceed Int32.MaxValue is unreachable"
            }
            ...
            test "flowField gives no arrow to an equal-valued neighbour (adjacent goals are both sinks)" {
                ...
                let field =
                    Pathfinding.distanceField FourWay 1000 cost [ { Col = 0; Row = 0 }; { Col = 1; Row = 0 } ]
                ...
                Expect.isFalse
                    (Map.containsKey { Col = 0; Row = 0 } flow)
                    "a goal with an equal-valued neighbour is still a sink"

                Expect.isFalse
                    (Map.containsKey { Col = 1; Row = 0 } flow)
                    "a goal with an equal-valued neighbour is still a sink"

                Expect.equal
                    (Map.tryFind { Col = 2; Row = 0 } flow)
                    (Some { Col = 1; Row = 0 })
                    "non-goals still step downhill"

                Expect.equal
                    (rollDownhill flow 8 { Col = 3; Row = 0 })
                    (Some { Col = 1; Row = 0 })
                    "rolling downhill arrives, it does not cycle"
            }
            ...
            test "flowField refuses to cut a corner past a cell absent from the field" {
                ...
                Expect.equal
                    (Map.tryFind { Col = 0; Row = 0 } flow)
                    (Some { Col = 0; Row = 1 })
                    "routes around, never diagonally past the missing corner"
            }
            ...
            test "reachableWithin charges the cell stepped onto (the forward direction)" {
                ...
                Expect.notEqual
                    (Map.tryFind { Col = 1; Row = 0 } range)
                    (Map.tryFind { Col = 1; Row = 0 } field)
                    "the two directions disagree on value under non-uniform cost"

                Expect.equal
                    (range |> Map.toList |> List.map fst)
                    (field |> Map.toList |> List.map fst)
                    "...but agree on which cells are reachable"
            }
            ...
            test "reachableWithin totality: a negative budget, an impassable start, or maxVisited <= 0" {
                ...
                Expect.equal
                    (Pathfinding.reachableWithin FourWay 5000 cost -1 { Col = 1; Row = 1 })
                    Map.empty
                    "negative budget"

                Expect.equal
                    (Pathfinding.reachableWithin FourWay 5000 cost 100 { Col = 0; Row = 0 })
                    Map.empty
                    "impassable start"

                Expect.equal
                    (Pathfinding.reachableWithin FourWay 0 cost 100 { Col = 1; Row = 1 })
                    Map.empty
                    "zero maxVisited"

                Expect.equal
                    (Pathfinding.reachableWithin FourWay -1 cost 100 { Col = 1; Row = 1 })
                    Map.empty
                    "negative maxVisited"

                Expect.equal
                    (Pathfinding.reachableWithin FourWay 5000 cost 0 { Col = 1; Row = 1 })
                    (Map.ofList [ { Col = 1; Row = 1 }, 0 ])
                    "zero budget ⇒ stand still"
            }
            ...
            test "reachableWithin terminates on an unbounded cost predicate (maxVisited, not budget, bounds it)" {
                ...
                let range =
                    Pathfinding.reachableWithin FourWay 200 (fun _ -> 1) 100_000 { Col = 0; Row = 0 }

                Expect.equal (Map.count range) 200 "exactly maxVisited cells are settled"

                Expect.equal
                    (Map.tryFind { Col = 0; Row = 0 } range)
                    (Some 0)
                    "the start is settled first, and costs nothing"

                Expect.all
                    (range |> Map.toList |> List.map snd)
                    (fun v -> v >= 0 && v <= 100_000)
                    "every settled value is within budget"
            }
            ...
            test "you may path THROUGH an ally but not end ON them (§4.4's v1 rule)" {
                ...
                let reach =
                    Pathfinding.reachable FourWay 5000 cost (fun c -> c <> ally) 100 { Col = 0; Row = 0 }
            ...
            test "canEndOn narrows destinations only — including the unit's own cell, with no exception" {
                ...
                let reach =
                    Pathfinding.reachable FourWay 5000 cost (fun c -> not (occupied c)) 100 start
                ...
                let reach2 =
                    Pathfinding.reachable FourWay 5000 cost (fun c -> c = start || not (occupied c)) 100 start
            ...
            test "pathTo totality: the start, an unsettled cell, and the CameFrom root" {
                ...
                let settledButOne =
                    reach.Steps
                    |> Map.toList
                    |> List.filter (fun (_, s) -> s.CameFrom.IsNone)
                    |> List.length
            ...
            test "reachable totality: a negative budget, an impassable start, or maxVisited <= 0" {
                ...
                let empty =
                    {
                        Pathfinding.Steps = Map.empty
                        Pathfinding.Endable = Set.empty
                    }

                let go maxVisited budget start =
                    Pathfinding.reachable FourWay maxVisited cost (fun _ -> true) budget start
                ...
                Expect.equal
                    (standStill.Steps |> Map.toList |> List.map fst)
                    [ { Col = 1; Row = 1 } ]
                    "zero budget ⇒ stand still"
            ...
            test "EightWay + non-uniform cost: a settled cell's CameFrom must track the cost that WON" {
                ...
                Expect.equal
                    (reach.Steps.[mud].Cost)
                    40
                    "the orthogonal route is cheaper than the diagonal one (40 < 42)"

                Expect.notEqual
                    (reach.Steps.[mud].CameFrom)
                    (Some start)
                    "so CameFrom must NOT still point at the diagonal predecessor"
            ...
            testCase "every settled path costs exactly what the highlight promised, and fits the budget (FsCheck)"
            <| fun () ->
                ...
                let prop (mudRaw: int) (blockedRaw: int) (b: int) (eightWay: bool) =
                    let bitAt (mask: int) (c: Cell) =
                        (mask >>> (c.Row * 5 + c.Col)) &&& 1 = 1
                    ...
                    if cost start <= 0 then
                        ...
                        let endableIsASubset =
                            reach.Endable =
                                (reach.Steps |> Map.toList |> List.map fst |> List.filter canEndOn |> Set.ofList)

                        consistent && endableIsASubset

                Check.One(Config.QuickThrowOnFailure.WithMaxTest 500, prop)

            testCase "reachableWithin is exactly reachable's Steps with the predecessors dropped (FsCheck)"
            <| fun () ->
            ...
            testCase "distanceField agrees with astar's path cost for every reachable cell (FsCheck)"
            <| fun () ->
            ...
            testCase "a cell is in the distanceField exactly when astar can reach the goal (FsCheck)"
            <| fun () ->
                let prop (blockedRaw: (int * int) list) (gc: int) (gr: int) =
                    let blocked =
                        blockedRaw
                        |> List.map (fun (c, r) -> (((abs c) % 5), ((abs r) % 5)))
                        |> Set.ofList
            ...
            testCase "reachableWithin equals distanceField filtered by budget, under uniform cost (FsCheck)"
            <| fun () ->
            ...
            testCase "reachableWithin is a subset of distanceField, under any cost (FsCheck)"
            <| fun () ->
                let prop (blockedRaw: (int * int) list) (mudRaw: (int * int) list) (sc: int) (sr: int) (b: int) =
                    ...
                    let mud =
                        mudRaw |> List.map (fun (c, r) -> (((abs c) % 6), ((abs r) % 6))) |> Set.ofList
            ...
            testCase "rolling downhill on the flow field always arrives at a goal (FsCheck)"
            <| fun () ->
                let prop (blockedRaw: (int * int) list) (gc: int) (gr: int) =
                    let blocked =
                        blockedRaw
            ...
            testCase "the fields are byte-identical across repeat runs over random terrain (FsCheck)"
            <| fun () ->
                ...
                let prop (blockedRaw: (int * int) list) (mudRaw: (int * int) list) (goalsRaw: (int * int) list) =
                    ...
                    let mud =
                        mudRaw |> List.map (fun (c, r) -> (((abs c) % 7), ((abs r) % 7))) |> Set.ofList

                    let goals =
                        goalsRaw |> List.map (fun (c, r) -> { Col = (abs c) % 7; Row = (abs r) % 7 })
            ...
            test "baseStep IS the scale the search charges: orthogonal costs it, diagonal 14/10 of it" {
                ...
                let reach =
                    Pathfinding.reachable EightWay 5000 cost (fun _ -> true) (Pathfinding.budgetFor 3) start
            ...
            test "budgetFor preserves reachable's totality: negative stays empty, zero settles start alone" {
                ...
                let negative =
                    Pathfinding.reachable FourWay 5000 cost (fun _ -> true) (Pathfinding.budgetFor -1) start
                ...
                let zero =
                    Pathfinding.reachable FourWay 5000 cost (fun _ -> true) (Pathfinding.budgetFor 0) start
    ...
    && path
       ...
       |> List.forall (fun (a, b) ->
           if a.Col <> b.Col && a.Row <> b.Row then
               isWalkable { Col = b.Col; Row = a.Row }
               && isWalkable { Col = a.Col; Row = b.Row }
           else
               true)
...
let jpsTests =
    testList
        "Game.Core Pathfinding JPS (015, FR-001..FR-006)"
        [
            ...
            test "jps FourWay jumps a straight corridor to the goal" {
                ...
                let path =
                    Pathfinding.jps FourWay 1000 walk { Col = 0; Row = 0 } { Col = 5; Row = 0 }
            ...
            test "jps EightWay takes the strictly-cheaper diagonal on an open grid" {
                ...
                let path =
                    Pathfinding.jps EightWay 1000 walk { Col = 0; Row = 0 } { Col = 3; Row = 3 }
            ...
            test "jps EightWay refuses to cut the corner past a blocked orthogonal (D5)" {
                ...
                let path =
                    Pathfinding.jps EightWay 1000 walk { Col = 0; Row = 0 } { Col = 1; Row = 1 }

                let astar =
                    Pathfinding.astar EightWay 1000 walk { Col = 0; Row = 0 } { Col = 1; Row = 1 }

                Expect.equal
                    (path |> Option.map pathCost)
                    (astar |> Option.map pathCost)
                    "same cost as astar (no corner-cut)"

                Expect.isTrue
                    (validPathNoCut walk { Col = 0; Row = 0 } { Col = 1; Row = 1 } (Option.get path))
                    "no corner-cut ⇒ routes around"
            }
            ...
            test "jps routes around a wall with a single gap (FR-002 validity)" {
                ...
                Expect.isTrue
                    (validPathNoCut walk start goal (Option.get path))
                    "valid walkable adjacent no-corner-cut path"
            ...
            test "jps shares astar's degenerate-input contract (FR-004 totality)" {
                ...
                Expect.isNone
                    (Pathfinding.jps FourWay 100 walk { Col = 0; Row = 0 } { Col = 1; Row = 1 })
                    "blocked start ⇒ None"

                Expect.isNone
                    (Pathfinding.jps FourWay 100 walk { Col = 1; Row = 1 } { Col = 2; Row = 2 })
                    "blocked goal ⇒ None"

                Expect.isNone
                    (Pathfinding.jps FourWay 0 open3 { Col = 0; Row = 0 } { Col = 2; Row = 2 })
                    "maxVisited 0 ⇒ None"

                Expect.isNone
                    (Pathfinding.jps FourWay -1 open3 { Col = 0; Row = 0 } { Col = 2; Row = 2 })
                    "negative maxVisited ⇒ None"

                Expect.equal
                    (Pathfinding.jps EightWay 100 open3 { Col = 1; Row = 1 } { Col = 1; Row = 1 })
                    (Some [ { Col = 1; Row = 1 } ])
                    "start = goal ⇒ [start]"
            }
            ...
            test "jps agrees with astar that a walled-off goal is unreachable (FR-003)" {
                ...
                Expect.isNone
                    (Pathfinding.jps EightWay 5000 walk { Col = 0; Row = 0 } { Col = 4; Row = 5 })
                    "goal enclosed ⇒ None"
            }
            ...
            test "determinism: jps is byte-identical across repeat runs (golden)" {
                ...
                Expect.isTrue
                    (a |> Option.get |> validPathNoCut walk { Col = 0; Row = 0 } { Col = 1; Row = 1 })
                    "and it is a valid path"
            }
            ...
            test "FR-006: hotspot decomposition baseline (#492): jps reaches the goal on a budget that starves astar" {
                ...
                Expect.isNone
                    (Pathfinding.astar FourWay budget walk start goal)
                    "astar exhausts the same budget mid-triangle"
            ...
            testCase "differential: jps equals astar in cost + validity + reachability over random grids (FsCheck)"
            <| fun () ->
            ...
            testCase "determinism: jps repeat calls are byte-identical over random grids (FsCheck)"
            <| fun () ->
                let prop (blockedRaw: (int * int) list) (sc: int) (sr: int) (gc: int) (gr: int) =
                    let blocked =
                        blockedRaw
...
let regionsTests =
    testList
        "Game.Core Pathfinding Regions (016, FR-001..FR-005)"
        [
            ...
            test "sameComponent separates two islands and joins within each (FR-001)" {
                ...
                let regions =
                    Pathfinding.Regions.build FourWay ({ Col = 0; Row = 0 }, { Col = 4; Row = 2 }) walk

                Expect.isTrue
                    (Pathfinding.Regions.sameComponent regions { Col = 0; Row = 0 } { Col = 1; Row = 2 })
                    "same (left) island"

                Expect.isTrue
                    (Pathfinding.Regions.sameComponent regions { Col = 3; Row = 0 } { Col = 4; Row = 2 })
                    "same (right) island"

                Expect.isFalse
                    (Pathfinding.Regions.sameComponent regions { Col = 0; Row = 0 } { Col = 4; Row = 0 })
                    "across the wall ⇒ different components"
            }
            ...
            test "an out-of-bounds or unwalkable cell is in no component — even against itself (FR-002)" {
                ...
                let regions =
                    Pathfinding.Regions.build FourWay ({ Col = 0; Row = 0 }, { Col = 2; Row = 2 }) walk

                Expect.isFalse
                    (Pathfinding.Regions.sameComponent regions { Col = 1; Row = 1 } { Col = 1; Row = 1 })
                    "unwalkable cell: false even against itself"

                Expect.isFalse
                    (Pathfinding.Regions.sameComponent regions { Col = 9; Row = 9 } { Col = 0; Row = 0 })
                    "out-of-bounds ⇒ false"

                Expect.isTrue
                    (Pathfinding.Regions.sameComponent regions { Col = 0; Row = 0 } { Col = 0; Row = 0 })
                    "a walkable cell IS in its own component"
            }
            ...
            test "EightWay connectivity honours no corner-cutting, matching astar (FR-003)" {
                ...
                let regions =
                    Pathfinding.Regions.build EightWay ({ Col = 0; Row = 0 }, { Col = 1; Row = 1 }) walk

                let astarJoins =
                    (Pathfinding.astar EightWay 1000 walk { Col = 0; Row = 0 } { Col = 1; Row = 1 }).IsSome

                Expect.equal
                    (Pathfinding.Regions.sameComponent regions { Col = 0; Row = 0 } { Col = 1; Row = 1 })
                    astarJoins
                    "no corner-cut ⇒ not connected, exactly as astar"
            ...
            test "determinism: Regions yields byte-identical answers across repeat builds (golden)" {
                ...
                let pairs =
                    [
                        for a in 0..3 do
                            for b in 0..3 -> ({ Col = a; Row = b }, { Col = b; Row = a })
                    ]

                Expect.isTrue
                    (pairs
                     |> List.forall (fun (a, b) ->
                         Pathfinding.Regions.sameComponent r1 a b = Pathfinding.Regions.sameComponent r2 a b))
            ...
            test "sameComponent needs no search bound to reject an unreachable pair (FR-005)" {
                ...
                let regions =
                    Pathfinding.Regions.build FourWay ({ Col = 0; Row = 0 }, { Col = 9; Row = 9 }) walk

                Expect.isFalse
                    (Pathfinding.Regions.sameComponent regions { Col = 0; Row = 0 } { Col = 9; Row = 9 })
                    "walled apart ⇒ rejected with no exploration"
            }

            testCase "differential: sameComponent equals astar reachability over random bounded grids (FsCheck)"
            <| fun () ->
                let prop (blockedRaw: (int * int) list) (eightWay: bool) =
                    ...
                    let regions =
                        Pathfinding.Regions.build nb ({ Col = 0; Row = 0 }, { Col = 5; Row = 5 }) walk

                    let cells =
                        [
                            for c in 0..5 do
                                for r in 0..5 -> { Col = c; Row = r }
                        ]
            ...
            testCase "determinism: Regions is a pure function of its inputs over random terrain (FsCheck)"
            <| fun () ->
...
let landmarksTests =
    testList
        "Game.Core Pathfinding Landmarks/ALT (017, FR-001..FR-006)"
        [
            ...
            test "Landmarks.astar returns an optimal-cost, valid path (FR-001/FR-002)" {
                ...
                let lm =
                    Pathfinding.Landmarks.build EightWay walk 4 ({ Col = 0; Row = 0 }, { Col = 4; Row = 4 })
            ...
            test "the ALT heuristic is admissible — never exceeds the true distance (FR-005)" {
                ...
                for cell in cells do
                    ...
                    | Some p ->
                        Expect.isTrue
                            (Pathfinding.Landmarks.heuristic lm goal cell <= pathCost p)
                            (sprintf "admissible at %A" cell)
            ...
            test "determinism: build + Landmarks.astar are byte-identical across runs (golden)" {
                ...
                Expect.equal
                    (Pathfinding.Landmarks.astar lm1 EightWay 5000 walk start goal)
                    (Pathfinding.Landmarks.astar lm2 EightWay 5000 walk start goal)
                    "identical inputs ⇒ identical path"
                ...
                Expect.isTrue
                    (cells
                     |> List.forall (fun c ->
                         Pathfinding.Landmarks.heuristic lm1 goal c = Pathfinding.Landmarks.heuristic lm2 goal c))
                    "and identical heuristics"
            }
            ...
            test "FR-006: Landmarks.astar expands strictly fewer nodes than astar on a detour map" {
                ...
                let altPops =
                    firstSolve (fun b -> Pathfinding.Landmarks.astar lm FourWay b walk start goal)

                Expect.isLessThan
                    altPops
                    astarPops
                    (sprintf "ALT expands fewer nodes (ALT=%d, astar=%d)" altPops astarPops)
            }

            testCase "differential: Landmarks.astar equals astar in cost + validity + reachability (FsCheck)"
            <| fun () ->
                let prop (blockedRaw: (int * int) list) (sc: int) (sr: int) (gc: int) (gr: int) (eightWay: bool) =
                    let blocked =
                        blockedRaw
                    ...
                    let lm =
                        Pathfinding.Landmarks.build nb walk 4 ({ Col = 0; Row = 0 }, { Col = 6; Row = 6 })
            ...
            testCase "admissibility over random grids: heuristic never overestimates (FsCheck)"
            <| fun () ->
                let prop (blockedRaw: (int * int) list) (gc: int) (gr: int) =
                    let blocked =
                        blockedRaw
                    ...
                    let lm =
                        Pathfinding.Landmarks.build EightWay walk 4 ({ Col = 0; Row = 0 }, { Col = 5; Row = 5 })
...
let straightTests =
    testList
        "Game.Core Pathfinding astarStraight (018, FR-001..FR-005)"
        [
            ...
            test "FR-004: astarStraight hugs the line — smaller deviation than astar on an open off-axis query" {
                ...
                Expect.isLessThan
                    (lineDeviation start goal s)
                    (lineDeviation start goal a)
                    "astarStraight deviates less from the straight line"
            ...
            test "astarStraight cross-product does not overflow/throw at extreme coordinates (totality)" {
                ...
                Expect.isNone
                    (Pathfinding.astarStraight FourWay 50 walk start goal)
                    "far goal unreachable within budget — and reached without throwing"
                ...
                | Some p ->
                    Expect.equal (pathCost p) 30 "3 orthogonal steps × baseStep — shortest route at extreme coords"
                | None -> failtest "expected a path between two near cells at extreme coordinates"
            }
            ...
            testCase "differential: astarStraight equals astar in cost + validity + reachability (FsCheck)"
            <| fun () ->
                let prop (blockedRaw: (int * int) list) (sc: int) (sr: int) (gc: int) (gr: int) (eightWay: bool) =
                    let blocked =
                        blockedRaw
            ...
            testCase "astarStraight deviation is never worse than astar's over random open grids (FsCheck)"
            <| fun () ->
                ...
                let prop (sc: int) (sr: int) (gc: int) (gr: int) =
                    ...
                    match
                        Pathfinding.astarStraight FourWay 5000 walk start goal,
                        Pathfinding.astar FourWay 5000 walk start goal
                    with
                    | Some s, Some a ->
                        pathCost s = pathCost a
                        && lineDeviation start goal s <= lineDeviation start goal a
                    | None, None -> true
                    | _ -> false
...
let smoothTests =
    testList
        "Game.Core Pathfinding smooth (021, FR-001..FR-005)"
        [
            ...
            test "totality: empty and single-cell paths are unchanged (FR-004)" {
                ...
                Expect.equal
                    (Pathfinding.smooth los [ { Col = 2; Row = 2 } ])
                    [ { Col = 2; Row = 2 } ]
                    "single ⇒ itself"
            }
            ...
            test "smooth keeps the corner it must turn around a wall (FR-002/FR-005)" {
                ...
                Expect.isTrue
                    ...
                    "every segment is LOS-clear"
            }

            testCase "smooth is a subsequence with LOS-clear segments, never longer (FsCheck)"
            <| fun () ->
                let prop (blockedRaw: (int * int) list) (sc: int) (sr: int) (gc: int) (gr: int) =
                    let blocked =
                        blockedRaw
            ...
            testCase "determinism: smooth is byte-identical across runs (FsCheck)"
            <| fun () ->
                let prop (blockedRaw: (int * int) list) (sc: int) (sr: int) (gc: int) (gr: int) =
                    let blocked =
                        blockedRaw
...
let bucketedPqGateTests =
    testList
        "Game.Core Pathfinding bucketed-PQ gate (026, FR-001)"
        [
            ...
            test "bucketed-PQ gate: astar solves a large search optimally within budget (Set frontier is adequate)" {
                ...
                | Some p ->
                    ...
                    Expect.equal
                        (pathCost p)
                        (119 * 14)
                        "optimal diagonal cost — the Set frontier finds it within budget"
