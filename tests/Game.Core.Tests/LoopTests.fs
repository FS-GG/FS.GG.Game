...
let private tick (w: Trace) (dt: float) =
    {
        Ticks = w.Ticks + 1
        Dts = w.Dts @ [ dt ]
    }
...
let tests =
    testList
        "Game.Core Loop double step buffer (US3, FR-009/FR-010)"
        [
            ...
            testList
                "totality — a non-finite input degrades; it never wedges the loop (FS-GG/FS.GG.Rendering#266)"
                [
                        ...
                        let expected =
                            [
                                ...
                                } // 3/32 = one step + 1/32
                                {
                                    ...
                                    Previous = 0.5625
                                    Accumulator = 0.03125
                                } // 5.0 clamped to 0.25 ⇒ four
                                {
                                    Current = 0.625
                                    Previous = 0.5625
                                    Accumulator = 0.046875
                                } // 1/32 + 1/64 < a step: bank
                                {
                                    Current = 0.6875
                                    Previous = 0.625
                                    Accumulator = 0.015625
                                }
                            ] // 3/64 + 1/32 ⇒ one step
