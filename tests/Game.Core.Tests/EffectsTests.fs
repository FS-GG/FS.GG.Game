...
let tests =
    testList
        "Game.Core Effects (mitigation layer, damage pipeline + status policies)"
        [
            ...
            testList
                "the floor never resurrects a zero (the reason `Halt` exists)"
                [
                    ...
                    test "a fully frost-immune target takes 0 from a frost bolt, not the floor of 1" {
                        ...
                        Expect.equal
                            trace.Halted
                            (ValueSome "resist")
                            "the caller must be able to see WHY, to suppress the rider Slow"
                    }
                    ...
                    test "a resistance above 1.0 halts rather than multiplying by a negative and healing" {
                        ...
                        let trace =
                            Effects.pipeline towerDefense overResistant (hit Frost Source.Declared 10.0)
                    ...
                    test "a resistance of 0.999 does NOT halt — it multiplies, and the floor then applies" {
                        ...
                        Expect.equal
                            trace.Final
                            1.0
                            "0.005 raised to the floor of 1.0 — the floor is for the nearly-immune"
                    ...
                    test "a zero transport multiplier is a SEED, not a Halt — the floor still lifts it to 1" {
                        ...
                        Expect.equal
                            (dealt |> List.map (snd >> final))
                            [ 1.0 ]
                            "the documented sharp edge: a linearFalloff 0.0 rim target takes the floor"

                        Expect.equal
                            (dealt |> List.map (snd >> fun t -> t.Halted))
                            [ ValueNone ]
                            "nothing halted — which is exactly why the floor ran"
                    }
                    ...
                    test "…and an immunity DOES halt, so the floor never runs — the difference in one place" {
                        ...
                        let seeded =
                            Effects.applyAll towerDefense (hit Physical Source.Declared 30.0) [ baseUnit, 0.0 ]

                        let halted =
                            Effects.applyAll towerDefense (hit Frost Source.Declared 30.0) [ spectre, 1.0 ]
                    ...
                    test "a game closes the rim gap in its region operator, or with immuneWhen" {
                        ...
                        Expect.equal
                            (Effects.applyAll towerDefense damage pruned |> List.map (snd >> final))
                            [ 15.0 ]
                            "the rim target is not a target"

                        // (b) Or halt on it, which also suppresses any rider effect.
                        let grazed = Effects.immuneWhen (fun _ _ -> true)

                        Expect.equal
                            (Effects.pipeline (grazed :: towerDefense) baseUnit damage |> final)
                            0.0
                            "an explicit halt beats the floor"
                    }
                    ...
                    test "immuneWhen halts, and names itself, so a rider effect can be suppressed" {
                        ...
                        let trace =
                            Effects.pipeline [ ghost; Effects.floorAt 1.0 ] spectre (hit Physical Source.Declared 99.0)
            ...
            testList
                "order of operations (each stage pure, and separately testable)"
                [
                    ...
                    test "amplify multiplies by (1 + bonus)" {
                        ...
                        Expect.equal
                            (Effects.pipeline [ vuln ] u (hit Physical Source.Declared 30.0) |> final)
                            42.0
                            "30 × 1.4"
                    }
                    ...
                    test "resist multiplies by (1 - r)" {
                        ...
                        Expect.equal
                            (Effects.pipeline [ frostResist ] u (hit Frost Source.Declared 40.0) |> final)
                            30.0
                            "40 × 0.75"
                    }
                    ...
                    test "resist only bites the kind it is written against" {
                        ...
                        Expect.equal
                            (Effects.pipeline [ frostResist ] u (hit Physical Source.Declared 40.0) |> final)
                            40.0
                            "frost resist ignores a physical hit"
                    }
                    ...
                    test "subtract removes a flat amount, and may drive the running total negative" {
                        ...
                        Expect.equal
                            (Effects.pipeline [ armor ] u (hit Physical Source.Declared 4.0) |> final)
                            -2.0
                            "floorAt decides the bottom, not subtract"
                    }
                    ...
                    test "floorAt clamps up, and is the last word" {
                        ...
                        Expect.equal
                            (Effects.pipeline [ armor; Effects.floorAt 1.0 ] u (hit Physical Source.Declared 4.0)
                             |> final)
                            1.0
                            "tower-defense floor"

                        Expect.equal
                            (Effects.pipeline [ armor; Effects.floorAt 0.0 ] u (hit Physical Source.Declared 4.0)
                             |> final)
                            0.0
                            "tactics floor — a unit behind cover survives"
                    }
                    ...
                    test "scale BEFORE subtract: falloff must not shield the target from its own armor" {
                        ...
                        let atRim =
                            Effects.applyAll
                                [ armor; Effects.floorAt 1.0 ]
                                (hit Physical Source.Declared 30.0)
                                [ brute, 0.5 ]
                        ...
                        Expect.equal
                            (atRim |> List.map (snd >> final))
                            [ 9.0 ]
                            "max(1, 0.5·30 − 6) = 9 — NOT 0.5·max(1, 30 − 6) = 12"

                        Expect.equal
                            (atCentre |> List.map (snd >> final))
                            [ 24.0 ]
                            "the two orders agree at the centre; they disagree by a third at the rim"
                    }
                    ...
                    test "amplify BEFORE subtract: an anti-armor upgrade must overcome armor, not scale the leak" {
                        ...
                        let chosen =
                            Effects.pipeline
                                [ vuln; armor; Effects.floorAt 1.0 ]
                                brute
                                (hit Physical Source.Declared 30.0)

                        let rejected =
                            Effects.pipeline
                                [ armor; vuln; Effects.floorAt 1.0 ]
                                brute
                                (hit Physical Source.Declared 30.0)
                        ...
                        Expect.floatClose
                            ...
                            rejected.Final
                            33.6
                            "max(1, 24) × 1.4 — it merely scales what armor let through"

                        Expect.isTrue
                            (chosen.Final > rejected.Final)
                            "and the difference is the whole point of the upgrade"
                    }
                ]
            ...
            testList
                "gatedBy (damage source is an axis, and it is load-bearing)"
                [
                    ...
                    test "armor-bypass is keyed on Kind, not Source: a Declared Frost bolt still bypasses armor" {
                        ...
                        let bySource =
                            [
                                Effects.gatedBy [ Source.Declared; Source.Collision ] armor
                                Effects.floorAt 1.0
                            ] // the trap

                        Expect.equal
                            (Effects.pipeline byKind brute (hit Frost Source.Declared 4.0) |> final)
                            4.0
                            "a Frost bolt ignores armor"

                        Expect.equal
                            (Effects.pipeline bySource brute (hit Frost Source.Declared 4.0) |> final)
                            1.0
                            "the source gate armors it — the bug"

                        Expect.equal
                            (Effects.pipeline byKind brute (hit Physical Source.Declared 4.0) |> final)
                            1.0
                            "a Physical 4 against armor 6 is floored"
                    }
                    ...
                    test "a Periodic tick and a Declared bolt of the same elemental Kind agree — no Source gate needed" {
                        ...
                        Expect.equal
                            (Effects.pipeline p brute (hit Frost Source.Periodic 4.0) |> final)
                            4.0
                            "poison tick bypasses armor"

                        Expect.equal
                            (Effects.pipeline p brute (hit Frost Source.Declared 4.0) |> final)
                            4.0
                            "and so does the bolt that applied it"
                    }
                    ...
                    test "gatedBy is for the rules that ARE about Source: cover and Armored" {
                        ...
                        Expect.equal
                            (Effects.pipeline tbt crawler (hit Physical Source.Declared 3.0) |> final)
                            0.0
                            "3 − 2 cover − 1 armored"

                        Expect.equal
                            (Effects.pipeline tbt crawler (hit Physical Source.Declared 1.0) |> final)
                            0.0
                            "AC#3 — max(0, 1 − 2) lets the Crawler survive the Ram"

                        Expect.equal
                            (Effects.pipeline tbt crawler (hit Physical Source.Collision 1.0) |> final)
                            1.0
                            "shoved into a wall: neither applies"

                        Expect.equal
                            (Effects.pipeline tbt crawler (hit Physical Source.Environmental 3.0) |> final)
                            3.0
                            "standing in lava: neither applies"
                    }
                    ...
                    test "a gated stage keeps the inner stage's name, fired or not, so traces are comparable" {
                        ...
                        Expect.equal
                            (skipped.Steps |> List.map fst)
                            [ "subtract" ]
                            "one entry per stage regardless of source"
            ...
            testList
                "DamageTrace (the audit is the feature)"
                [
                    ...
                    test "Steps records every stage's output, in order" {
                        ...
                        let trace =
                            Effects.pipeline
                                [ vuln; armor; Effects.floorAt 1.0 ]
                                brute
                                (hit Physical Source.Declared 10.0)

                        Expect.equal
                            trace.Steps
                            [ "amplify", 15.0; "subtract", 9.0; "floorAt", 9.0 ]
                    ...
                    test "Seed records the seeded amount, which Steps cannot" {
                        ...
                        let trace =
                            Effects.applyAll [ armor ] (hit Physical Source.Declared 30.0) [ brute, 0.5 ]
                            |> traceOf
                    ...
                    test "Seed is an amount, not a provenance — a ×0.5 blast of 30 and a ×1.0 blast of 15 trace alike" {
                        ...
                        let weakBlast =
                            Effects.applyAll [ armor ] (hit Physical Source.Declared 15.0) [ brute, 1.0 ]
                            |> traceOf

                        Expect.equal
                            halvedBlast
                            weakBlast
                            "identical in every field — after seeding they are the same hit"
                    ...
                    test "under pipeline, Seed is damage.Base" {
                        ...
                        let trace =
                            Effects.pipeline [ vuln; armor ] brute (hit Physical Source.Declared 10.0)

                        Expect.equal trace.Seed 10.0 "no transport multiplier ⇒ the seed is the base"

                        Expect.equal
                            trace.Steps
                            [ "amplify", 15.0; "subtract", 9.0 ]
                            "amplify's 15.0 is a Step; the seed is not"
                    }
                    ...
                    test "a non-finite seed degrades to 0.0 rather than reporting NaN" {
                        ...
                        let byMultiplier =
                            Effects.applyAll [ armor ] (hit Physical Source.Declared 30.0) [ baseUnit, infinity ]
                            |> traceOf
            ...
            testList
                ...
                [
                    ...
                    <| fun () ->
                        let prop (armors: int list) (seed: int) =
                            let targets =
                                ...
                                |> List.mapi (fun i a ->
                                    { baseUnit with
                                        Armor = float (abs a % 20)
                                    },
                                    0.25 + float ((i + abs seed) % 4) * 0.25)
                            ...
                            let inIsolation =
                                targets
                                |> List.map (fun t -> Effects.applyAll towerDefense damage [ t ] |> List.head |> snd)
                    ...
                    <| fun () ->
                        let prop (armors: int list) (rot: int) =
                            ...
                            | _ ->
                                ...
                                let rotatedBack =
                                    List.skip (List.length permuted - k) permuted
                                    @ List.take (List.length permuted - k) permuted
                    ...
                    test "applyAll returns results in the order the targets were given" {
                        ...
                        let dealt =
                            Effects.applyAll [ armor ] (hit Physical Source.Declared 10.0) [ a, 1.0; b, 1.0 ]
                    ...
                    test "applyAll SEEDS with the transport multiplier — it is not a stage in the list" {
                        ...
                        Expect.equal
                            (dealt |> List.map (snd >> final))
                            [ 9.0 ]
                            "15 − 6; the multiplier cannot be reordered after subtract"

                        Expect.equal
                            (dealt |> List.collect (snd >> fun t -> t.Steps))
                            [ "subtract", 9.0 ]
                            "the seed is not a Step"
                    }

                    test "applyAll over no targets is empty — a blast that caught nobody" {
                        Expect.isEmpty
                            (Effects.applyAll towerDefense (hit Physical Source.Declared 30.0) [])
                            "no targets, no traces"
                    }
                ]
            ...
            testList
                "the AoE models compose as (region operator) × (pipeline)"
                [
                    ...
                    test "tower-defense cannon splash: Ballistics.splash × [amplify; resist; armor; floor 1]" {
                        ...
                        let grid =
                            SpatialGrid.build 32.0 [ { X = 0.0; Y = 0.0 }, brute; { X = 42.0; Y = 0.0 }, brute ]
                        ...
                        Expect.equal
                            (dealt |> List.map (snd >> final))
                            [ 24.0; 24.0 ]
                            "both projected to the centre ⇒ multiplier 1.0"
                    }
                    ...
                    test "tower-defense Tesla chain: the scalar is chainFalloff^k in the JUMP INDEX, not distance" {
                        ...
                        let dealt =
                            Effects.applyAll
                                [ frostResist; Effects.floorAt 1.0 ]
                                (hit Frost Source.Declared 100.0)
                                region

                        Expect.equal
                            (dealt |> List.map (snd >> final))
                            [ 100.0; 60.0; 36.0 ]
                            "the k-th jump takes 0.6^k"
                    }
                    ...
                    test "missile-command blast: SpatialGrid.queryRadius × [] — membership IS the mechanic" {
                        ...
                        let region =
                            SpatialGrid.queryRadius { X = 0.0; Y = 0.0 } 40.0 grid
                            |> List.map (fun u -> u, 1.0)
                        ...
                        Expect.equal
                            (dealt |> List.map (snd >> final))
                            [ 1.0 ]
                            "any positive amount is lethal; there is nothing to mitigate"
                    }
                    ...
                    test "turn-based-tactics Blast r: a uniform scalar × [cover; armored; floor 0]" {
                        ...
                        Expect.equal
                            (dealt |> List.map (snd >> final))
                            [ 0.0; 0.0 ]
                            "cover 2 and armor 1 both stop a Ram of 1"
                    }
                ]
            ...
            testList
                "status-effect policies (the coin flip every spec leaves unflipped)"
                [

                    testList
                        "Refresh (Stun: one instance, duration extends but never shortens)"
                        [
                            test "onto an empty list, applies" {
                                Expect.equal
                                    (Effects.applyEffect Effects.Refresh (active 1 30) [])
                                    [ active 1 30 ]
                                    "first application"
                            }
                            test "a longer reapplication extends" {
                                Expect.equal
                                    (Effects.applyEffect Effects.Refresh (active 1 30) [ active 1 10 ])
                                    [ active 1 30 ]
                                    "max(10, 30)"
                            }
                            test "a shorter reapplication does not cut the running stun short" {
                                Expect.equal
                                    (Effects.applyEffect Effects.Refresh (active 1 5) [ active 1 20 ])
                                    [ active 1 20 ]
                                    "max(20, 5)"
                            }
                        ]
                    ...
                    testList
                        "Strongest (Slow: the >= is what lets a second tower re-up a slow)"
                        [
                            test "onto an empty list, applies" {
                                Expect.equal
                                    (Effects.applyEffect strongest (active 5 10) [])
                                    [ active 5 10 ]
                                    "first application"
                            }
                            test "a stronger application replaces and refreshes" {
                                Expect.equal
                                    (Effects.applyEffect strongest (active 8 30) [ active 5 20 ])
                                    [ active 8 30 ]
                                    "8 > 5"
                            }
                            test "an EQUAL application refreshes — this is the load-bearing >=" {
                                Expect.equal
                                    (Effects.applyEffect strongest (active 5 30) [ active 5 4 ])
                                    [ active 5 30 ]
                                    "a second Frost tower re-ups the same slow"
                            }
                            test "a weaker application changes nothing at all" {
                                Expect.equal
                                    (Effects.applyEffect strongest (active 2 99) [ active 5 4 ])
                                    [ active 5 4 ]
                                    "not even the duration"
                            }
                            ...
                            test "an equal-magnitude SHORTER reapplication does not cut the running slow short" {
                                Expect.equal
                                    (Effects.applyEffect strongest (active 5 5) [ active 5 80 ])
                                    [ active 5 80 ]
                                    "max(80, 5) — the 75 ticks do not evaporate"
                            }
                            test "a STRONGER but shorter application replaces the effect and keeps the longer duration" {
                                Expect.equal
                                    (Effects.applyEffect strongest (active 8 5) [ active 5 80 ])
                                    [ active 8 80 ]
                                    "the incoming effect wins; only the duration is reconciled"
                            }
                    ...
                    testList
                        "Stacks cap (Poison: independent, additive, appended in order)"
                        [
                            ...
                            test "at the cap the application is a NO-OP — it does not refresh or evict" {
                                ...
                                Expect.equal
                                    (Effects.applyEffect (Effects.Stacks 3) (active 9 99) atCap)
                                    atCap
                                    "the documented choice; the source spec states none"
                            }
                            test "a cap of zero admits nothing" {
                                Expect.isEmpty
                                    (Effects.applyEffect (Effects.Stacks 0) (active 1 10) [])
                                    "cap 0 ⇒ no instances"
                            }
                        ]

                    testList
                        "Once (the first application wins, forever)"
                        [
                            test "onto an empty list, applies" {
                                Expect.equal
                                    (Effects.applyEffect Effects.Once (active 1 10) [])
                                    [ active 1 10 ]
                                    "first application"
                            }
                            test "a later, stronger, longer application is still a no-op" {
                                Expect.equal
                                    (Effects.applyEffect Effects.Once (active 9 99) [ active 1 1 ])
                                    [ active 1 1 ]
                                    "no-op"
                            }
                        ]
            ...
            testList
                "tickEffects (durations are int ticks; a tick is whatever the caller drains)"
                [

                    test "one tick decrements every instance" {
                        Expect.equal
                            (Effects.tickEffects [ active 1 3; active 2 2 ])
                            [ active 1 2; active 2 1 ]
                            "decrement, order-preserving"
                    }

                    test "an instance that reaches zero is dropped" {
                        Expect.equal
                            (Effects.tickEffects [ active 1 1; active 2 5 ])
                            [ active 2 4 ]
            ...
            testList
                "determinism golden (effects expire on the same step at any frame rate)"
                [
                    ...
                    let replay (frameTimes: float list) =
                        frameTimes
                        |> List.fold
                            (fun state ft -> Loop.advance dt integrate ft state)
                            (Loop.init ([ active 1 12; active 2 40 ], 0))
                        |> fun s -> s.Current
                    ...
                    test "a coarse and a fine frame script over the same wall time expire the same effects" {
                        ...
                        Expect.equal
                            activesCoarse
                            [ active 2 8 ]
                            "the 12-tick effect expired; the 40-tick one has 8 left"
                    }
                    ...
                    test "applyEffect and the pipeline are pure — the same inputs give the same outputs" {
                        ...
                        Expect.equal
                            (Effects.pipeline towerDefense brute d)
                            (Effects.pipeline towerDefense brute d)
            ...
            testList
                "totality (no input can put a NaN in a DamageTrace)"
                [
                    ...
                    test "a non-finite transport multiplier seeds 0.0" {
                        ...
                        |> List.iter (fun m ->
                            let dealt =
                                Effects.applyAll [ armor ] (hit Physical Source.Declared 30.0) [ baseUnit, m ]

                            Expect.equal
                                (dealt |> List.map (snd >> final))
                                [ 0.0 ]
                                $"0.0 seed, minus zero armor (multiplier = {m})")
                    }
                    ...
                    test "a hostile Stage cannot inject a NaN into the trace" {
                        let hostile: Stage<Unit, Kind> =
                            {
                                Name = "hostile"
                                Run = fun _ _ _ -> StageResult.Continue nan
                            }

                        let trace =
                            Effects.pipeline [ hostile; armor ] baseUnit (hit Physical Source.Declared 10.0)

                        Expect.isFalse (isNan trace.Final) "sanitised at the stage boundary"

                        Expect.equal
                            trace.Steps
                            [ "hostile", 0.0; "subtract", 0.0 ]
                            "the NaN became 0.0 before the next stage saw it"
                    }
                    ...
                    test "a hostile Stage cannot inject a NaN through a Halt either" {
                        let hostile: Stage<Unit, Kind> =
                            {
                                Name = "hostile"
                                Run = fun _ _ _ -> StageResult.Halt nan
                            }

                        let trace =
                            Effects.pipeline [ hostile ] baseUnit (hit Physical Source.Declared 10.0)
                    ...
                    test "floorAt with a non-finite minimum degrades to a floor of 0.0" {
                        ...
                        let trace =
                            Effects.pipeline [ armor; Effects.floorAt nan ] u (hit Physical Source.Declared 10.0)
                    ...
                    testCase
                        "no pipeline or applyAll over unclamped arbitrary floats yields a non-finite Seed, Final, or Step (FsCheck >=500)"
                    <| fun () ->
                        let prop (a: float) (r: float) (s: float) (f: float) (b: float) (m: float) =
                            ...
                            let traces =
                                Effects.pipeline stages u damage
                                :: (Effects.applyAll stages damage [ u, m ] |> List.map snd)
