    ...
    seq {
        ...
        for _ in 1..n do
            ...
            if len > 0.0 then
                ...
                if tHit > 0.0 then
                    let origin =
                        pt (ax + a * tx - sinTheta * tHit * nx) (ay + a * ty - sinTheta * tHit * ny)
    ...
    seq {
        ...
        for _ in 1..n do
            yield
                pt (coord ()) (coord ()),
                pt (coord ()) (coord ()),
                seg (pt (coord ()) (coord ())) (pt (coord ()) (coord ()))
    }

[<Tests>]
let tests =
    testList
        "Game.Core Visibility (continuous-space sight)"
        [

            // --- the regression this promotion carries -------------------------------------------------

            testCase "spanning chord occludes: the sweep sees nothing past a wall crossing its bound"
            <| fun () ->
                let poly =
                    Visibility.polygon (settings spanningRadius) spanningSource [ spanningWall ]
            ...
            testCase "spanning chord occludes: the ring turns at the wall's bound crossings"
            <| fun () ->
                let poly =
                    Visibility.polygon (settings spanningRadius) spanningSource [ spanningWall ]
                ...
                let has (x, y) =
                    poly.Vertices
                    |> List.exists (fun v -> abs (v.X - x) < 1e-6 && abs (v.Y - y) < 1e-6)
            ...
            testCase "spanning chord occludes: a target past the wall is not visible"
            <| fun () ->
                Expect.isFalse
                    (Visibility.isVisible spanningSource (pt 0.0 40.0) [ spanningWall ])
                    "blocked by the wall"
            ...
            testCase "the endpoint-bucket broad phase this replaced would have dropped that wall"
            <| fun () ->
                let endpointBuckets =
            ...
            testCase "raySegment finds the crossing and its parametric distance"
            <| fun () ->
                let hit =
                    Visibility.raySegment (pt 0.0 0.0) (pt 1.0 0.0) (seg (pt 5.0 -1.0) (pt 5.0 1.0))

                Expect.equal hit (Some(pt 5.0 0.0, 5.0)) "unit dir ⇒ t is the world distance"

            testCase "raySegment is None on parallel, zero-length, behind, and non-finite input"
            <| fun () ->
                let origin, dir = pt 0.0 0.0, pt 1.0 0.0
                Expect.isNone (Visibility.raySegment origin dir (seg (pt 1.0 1.0) (pt 5.0 1.0))) "parallel"

                Expect.isNone
                    (Visibility.raySegment origin dir (seg (pt 5.0 0.0) (pt 5.0 0.0)))
                    "zero-length occludes nothing"
                ...
                Expect.isNone
                    (Visibility.raySegment (pt nan 0.0) dir (seg (pt 5.0 -1.0) (pt 5.0 1.0)))
                    "non-finite origin"

            // --- isVisible (the "can this body shoot that body" predicate) -----------------------------

            testCase "isVisible: self, clear line, and a blocker strictly between"
            <| fun () ->
            ...
            testCase "isVisible: a blocker exactly at the target does not block (strict 0 < t < 1)"
            <| fun () ->
                let wall = seg (pt 5.0 -1.0) (pt 5.0 1.0)
                Expect.isTrue (Visibility.isVisible (pt 0.0 0.0) (pt 5.0 0.0) [ wall ]) "target sits on the wall"

            testCase "isVisible: non-finite endpoints are never visible"
            <| fun () ->
            ...
            testCase "polygon: a non-finite source yields an empty ring"
            <| fun () ->
            ...
            testCase "polygon: a non-positive or non-finite radius falls back to 1.0"
            <| fun () ->
            ...
            testCase "polygon: non-finite and zero-length occluders are discarded, not thrown on"
            <| fun () ->
                let junk =
                    [
                        seg (pt nan 0.0) (pt 1.0 1.0)
                        seg (pt 3.0 3.0) (pt 3.0 3.0)
                        seg (pt 0.0 infinity) (pt 1.0 1.0)
                    ]
            ...
            testCase "polygon is deterministic for identical input"
            <| fun () ->
            ...
            testCase "raySegment never yields a non-finite hit when the cross product overflows"
            <| fun () ->
            ...
            testCase "raySegment still finds the crossing when the far endpoint is representable"
            <| fun () ->
                let hit =
                    Visibility.raySegment (pt -3.937126736 0.0) (pt 0.937126736 0.0) (seg (pt 0.0 1.0e6) (pt 0.0 0.0))
            ...
            testCase "polygon emits no non-finite vertex for the #53 counterexample"
            <| fun () ->
                let walls =
                    [
                ...
                let poly =
                    Visibility.polygon (settings System.Double.MaxValue) (pt -3.937126736 0.0) walls
            ...
            testCase "raySegment recovers the representable hit when the numerator overflows"
            <| fun () ->
                let origin = pt -3.937126736 0.0
                let dir = pt 0.937126736 0.0
                let far = seg (pt 0.0 System.Double.MaxValue) (pt 0.0 0.0)
                ...
                let p, t =
                    Expect.wantSome (Visibility.raySegment origin dir far) "the crossing is representable"
            ...
            testCase "raySegment: the far endpoint's magnitude does not move the hit"
            <| fun () ->
                let origin = pt -3.937126736 0.0
                let dir = pt 0.937126736 0.0
            ...
            testCase "raySegment does not report a spurious t = 0 hit when the denominator overflows"
            <| fun () ->
                ...
                let p, t =
                    Expect.wantSome (Visibility.raySegment origin dir wall) "the crossing is representable"
            ...
            testCase "raySegment solves a wall spanning -MaxValue to +MaxValue"
            <| fun () ->
                let origin = pt -3.937126736 0.0
                let dir = pt 0.937126736 0.0
                ...
                let _, t =
                    Expect.wantSome (Visibility.raySegment origin dir wall) "an unbounded wall still occludes"
            ...
            testCase "the rescaled solve is deterministic for identical input"
            <| fun () ->
                let origin = pt -3.937126736 0.0
                let dir = pt 0.937126736 0.0
                let far = seg (pt 0.0 System.Double.MaxValue) (pt 0.0 0.0)
                ...
                let ring () =
                    Visibility.polygon (settings System.Double.MaxValue) origin walls
            ...
            testCase "raySegment's t is accurate to 1e-12 over a bounded world"
            <| fun () ->
            ...
            testCase "a grazing ray degrades exactly as its conditioning predicts, and no worse"
            <| fun () ->
                let mutable checked' = 0
                let mutable worst = 0.0
                ...
                Expect.isLessThan worst 1.0e-15 $"relErr * sinTheta (the conditioning law) over {checked'} grazing rays"
            ...
            testCase "outside the bounded world ~2% of hits carry a relative error above 1e-12"
            <| fun () ->
                let mutable checked' = 0
                ...
                for origin, dir, s in wideRangeSamples 20_000 do
                    ...
                    | Some(_, t) ->
                        ...
                        | Some exact ->
                            ...
                            if Oracle.relErr t exact > 1.0e-12 then
                                degraded <- degraded + 1
            ...
            testCase "the exact oracle reports zero error on an exact t, and sees a perturbation"
            <| fun () ->
                ...
                let _, t =
                    Expect.wantSome (Visibility.raySegment origin dir wall) "the ray crosses the wall"
            ...
            testCase "outside the promised regime raySegment is wrong, and the oracle says so"
            <| fun () ->
                let origin = pt 4.020794469e-09 0.4511871065
                let dir = pt 0.05228096296 -23.41834131

                let wall =
                    seg (pt -3.292173079e+199 -9.753634222e+99) (pt 5.213872332e-09 -0.6581832746)

                let hit, t =
                    Expect.wantSome (Visibility.raySegment origin dir wall) "a hit is still reported"
            ...
            testCase "raySegment: extending the occluder away from the crossing does not lose it (FsCheck)"
            <| fun () ->
                let prop (hx: float) (hy: float) (ox: float) (oy: float) (ex: float) (ey: float) =
                    ...
                    if
                        sqLenPt dir = 0.0
                        || elen = 0.0
                        || abs cross / (sqrt (sqLenPt dir) * elen) < 1.0e-3
                    then
                        true
                    else
                        ...
                        |> List.forall (fun scale ->
                            ...
                            if not (isFinitePt far) then
                                ...
                                | Some(p, t) ->
                                    ...
                                    abs (t - 1.0) <= 1.0e-6 && abs (p.X - h.X) <= tol && abs (p.Y - h.Y) <= tol)
            ...
            testCase "raySegment never returns a non-finite hit, on unclamped input (FsCheck)"
            <| fun () ->
                let prop
                    (ox: float)
                    (oy: float)
            ...
            testCase "polygon never throws and never emits a NaN vertex, on unclamped input (FsCheck)"
            <| fun () ->
                let prop (coords: (float * float * float * float) list) (sx: float) (sy: float) (r: float) =
                    let walls = coords |> List.map (fun (ax, ay, bx, by) -> seg (pt ax ay) (pt bx by))
            ...
            testCase "isVisible never throws, on unclamped input (FsCheck)"
            <| fun () ->
                let prop
                    (coords: (float * float * float * float) list)
                    (sx: float)
                    (sy: float)
                    (tx: float)
                    (ty: float)
            ...
            testCase "polygon: every vertex lies within the sight bound (FsCheck)"
            <| fun () ->
