...
let tests =
    testList
        "Game.Core Ai (US-42, FR-042..FR-046)"
        [
            ...
            test "known is ascending by AgentId regardless of input order" {
                let v =
                    Ai.view 10 50 [ sighting 5 0.0 0.0 10; sighting 1 0.0 0.0 3; sighting 3 0.0 0.0 10 ]
            ...
            test "a sighting from the future is clamped to spotted, not dropped" {
                ...
                Expect.equal
                    (Ai.spotted v |> List.map _.Agent)
                    [ AgentId 1 ]
                    "an agent that can see, not one gone blind"
            }
            ...
            test "distinct ids yield decorrelated streams" {
                ...
                let draws =
                    [ 0; 1; 2; -1; System.Int32.MinValue ]
                    |> List.map (fun id -> draw (Ai.substream root (AgentId id)))
            ...
            test "a non-finite dps contributes 0, and a negative range is out of range everywhere" {
                let f =
                    Ai.threatField (fun _ _ -> true) [ cell 0 0, nan, 5; cell 0 0, 7.0, -1 ] [ cell 0 0 ]
            ...
            <| fun () ->
                ...
                let prop (raw: (int * int * float * int) list) =
                    let sources =
                        raw
                        |> List.truncate 24
                        |> List.map (fun (c, r, dps, range) -> cell (c % 8) (r % 8), clampDps dps, abs (range % 6))

                    let cells =
                        [
                            for c in 0..3 do
                                for r in 0..3 -> cell c r
                        ]
                    ...
                    let rotated =
                        Ai.threatField
                            hasLos
                            (List.skip (List.length sources / 2) sources
                             @ List.truncate (List.length sources / 2) sources)
                            cells
            ...
            test "a non-finite coefficient yields a smaller field, not a NaN-poisoned one" {
                ...
                for bad in [ nan; infinity; -infinity ] do
                    ...
                    Expect.all
                        (f |> Map.toList |> List.map snd)
                        System.Double.IsFinite
                        $"coefficient {bad} returns no poison"
            }
            ...
            test "threatField does not wrap int64 when two cells sit at opposite Int32 extremes" {
                ...
                let f =
                    Ai.threatField (fun _ _ -> true) [ far, 100.0, System.Int32.MaxValue ] [ here ]
            ...
            test "GOLDEN: on a score tie, lowest target index then lowest position index (turn-based-tactics §4.9)" {
                ...
                Expect.equal
                    (pick |> ValueOption.map (fun (n, _, _, _) -> n))
                    (ValueSome "winner")
            ...
            test "best of an empty list is ValueNone" {
                Expect.equal
                    (Ai.best fst (fun _ -> struct (0, 0)) ([]: (float * int) list))
                    ValueNone
                    "no plans, no pick"
            }
            ...
            <| fun () ->
                ...
                let prop (scores: float list) =
                    ...
                    if plans.IsEmpty then
                        ...
                        let pick l =
                            Ai.best (fun (_, s, _, _) -> s) (fun (_, _, t, p) -> struct (t, p)) l
            ...
            test "the ladder is monotone in time and precision" {
                ...
                Expect.isGreaterThan
                    Difficulty.easy.SpotCycleTicks
                    Difficulty.hard.SpotCycleTicks
                    "hard looks more often"
...
module InfluenceMapTests =
    ...
    let influenceTests =
        testList
            "Game.Core Ai influenceMap (025, FR-001..FR-004)"
            [
                ...
                test "multiple sources combine by max (FR-002)" {
                    ...
                    let m =
                        Ai.influenceMap FourWay 5000 cost [ ({ Col = 0; Row = 0 }, 30); ({ Col = 10; Row = 0 }, 50) ]
                    ...
                    Expect.equal
                        (Map.tryFind { Col = 1; Row = 0 } m)
                        (Some 20)
                        "src1 contribution where src2 does not reach"
                }
                ...
                test "tension recipe: friendly minus enemy marks control (FR-004)" {
                    ...
                    let tension c =
                        (Map.tryFind c friendly |> Option.defaultValue 0)
                        - (Map.tryFind c enemy |> Option.defaultValue 0)
                ...
                testCase "influenceMap equals the brute-force per-source max over random sources (FsCheck)"
                <| fun () ->
                    let prop (srcsRaw: (int * int * int) list) =
                        ...
                        let sources =
                            srcsRaw
                            |> List.map (fun (c, r, st) ->
                                ({ Col = (abs c) % 7; Row = (abs r) % 7 }, 5 + (abs st) % 60))
                        ...
                        let fields =
                            sources
                            |> List.map (fun (s, st) -> (st, Pathfinding.distanceField FourWay 5000 cost [ s ]))

                        let cells =
                            [
                                for cc in 0..6 do
                                    for rr in 0..6 -> { Col = cc; Row = rr }
                            ]
                ...
                testCase "influenceMap is byte-deterministic over random sources (FsCheck)"
                <| fun () ->
                    let prop (srcsRaw: (int * int * int) list) =
                        let cost = gridCost 7 7
