...
let tests =
    testList
        "Game.Core Ballistics (US-40, FR-040..FR-044)"
        [
            ...
            test "a fast round hits a thin wall it crosses in one step — where a point test misses it" {
                ...
                Expect.isFalse
                    (Geometry.containsPoint wall after)
                    "the naive point test lands PAST the wall — it tunnels"
            ...
            test "lifetime is checked before motion — an exhausted round gets no free final step" {
                Expect.equal
                    (Ballistics.step noCast 1.0 (round (p 0.0 0.0) (p 5.0 0.0) 0))
                    Expired
                    "zero ticks is Expired"

                Expect.equal
                    (Ballistics.step noCast 1.0 (round (p 0.0 0.0) (p 5.0 0.0) -3))
                    Expired
                    "negative ticks too"
            }
            ...
            test "totality: a round that is already non-finite is Expired, so a NaN never escapes" {
                ...
                Expect.equal
                    (Ballistics.step noCast 1.0 (round (p 0.0 0.0) (p infinity 0.0) 5))
                    Expired
                    "infinite velocity"
            }
            ...
            test "a stationary target is intercepted where it stands" {
                Expect.equal
                    (Ballistics.intercept (p 0.0 0.0) 10.0 (p 3.0 4.0) (p 0.0 0.0))
                    (ValueSome(p 3.0 4.0))
                    "aim at it"
            }
            ...
            test "an outrunning target has no interception — ValueNone, not NaN" {
                Expect.equal
                    (Ballistics.intercept (p 0.0 0.0) 1.0 (p 10.0 0.0) (p 10.0 0.0))
                    ValueNone
                    "faster and fleeing"
            }
            ...
            test "a degenerate speed has no interception" {
                for bad in [ 0.0; -5.0; nan; infinity ] do
                    Expect.equal
                        (Ballistics.intercept (p 0.0 0.0) bad (p 3.0 0.0) (p 0.0 0.0))
                        ValueNone
                        (sprintf "speed=%f" bad)
            }
            ...
            test "intercept takes the EARLIEST of two positive roots" {
                ...
                | ValueSome aim ->
                    Expect.floatClose
                        ...
                        aim.X
                        (10.0 / 3.0)
                        "earliest root ⇒ x = 10/3, NOT the late root's -10"
            ...
            <| fun () ->
                ...
                Check.One(
                    Config.QuickThrowOnFailure.WithMaxTest(500).WithReplay(Some { Rnd = Rnd 39UL; Size = None }),
                    prop
                )
            ...
            testCase "the #89 counterexample: a unit-length flight between 1e6-magnitude operands"
            <| fun () ->
            ...
            testCase "the #97 phantom: a sub-unit outrunning target has no interception, not a linear-branch fiction"
            <| fun () ->
            ...
            testCase
                "a returned interception satisfies |aim - shooter| = speed * t at sub-unit speeds (#97, FsCheck >=500)"
            <| fun () ->
            ...
            test "splash pairs every item in radius with its falloff multiplier, in insertion order" {
                ...
                let hits =
                    Ballistics.splash (p 0.0 0.0) 10.0 (Ballistics.linearFalloff 0.5) fst grid
            ...
            test "a non-positive or non-finite radius splashes nothing" {
                ...
                for bad in [ 0.0; -1.0; nan; infinity ] do
                    Expect.isEmpty
                        (Ballistics.splash (p 0.0 0.0) bad (Ballistics.linearFalloff 0.5) fst grid)
                        (sprintf "radius=%f" bad)
            }
            ...
            test "a scripted dt sequence reproduces a byte-identical trajectory (golden)" {
                ...
                let expected = [ p 0.0 0.0; p 2.0 1.0; p 4.0 2.0; p 6.0 3.0; p 8.0 4.0; p 10.0 5.0 ]
