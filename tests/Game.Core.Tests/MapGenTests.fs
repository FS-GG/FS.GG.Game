    ...
    testList
        ...
        [
            ...
            <| fun () ->
                ...
                let m = tileMapOf [ [ 1; 1; 0 ]; [ 0; 0; 0 ]; [ 0; 0; 1 ] ]
            ...
            <| fun () ->
                ...
                let m = tileMapOf [ [ 1; 0; 1 ]; [ 0; 0; 0 ]; [ 1; 0; 0 ] ]
            ...
            <| fun () ->
                ...
                let gen (s: uint64) =
                    let struct (m, _) = MapGen.caves 32 24 p (Rng.ofSeed s) in m
            ...
            <| fun () ->
                let prop (w: int) (h: int) (wc: int) (sp: int) =
                    ...
                    let struct (_, _) =
                        MapGen.caves ((w % 20) - 4) ((h % 20) - 4) p (Rng.ofSeed (uint64 (abs wc)))
            ...
            <| fun () ->
                ...
                let overlaps (a: Room) (b: Room) =
                    let ax, ay, aw, ah =
                        int a.Bounds.X, int a.Bounds.Y, int a.Bounds.Width, int a.Bounds.Height

                    let bx, by, bw, bh =
                        int b.Bounds.X, int b.Bounds.Y, int b.Bounds.Width, int b.Bounds.Height
            ...
            <| fun () ->
                ...
                let valid (a, b) =
                    a >= 0 && b >= 0 && a < g.Rooms.Length && b < g.Rooms.Length
            ...
            <| fun () ->
                let prop (w: int) (h: int) (mn: int) (mx: int) =
                    ...
                    let struct (_, _, _) =
                        MapGen.bspDungeon ((w % 30) - 4) ((h % 30) - 4) p (Rng.ofSeed (uint64 (abs mx)))
            ...
            <| fun () ->
                let p: FloorParams =
                    {
                        RoomCount = 15
                        MaxRooms = 20
                        SpecialRooms = [ Boss; Treasure; Shop ]
                    }
                ...
                let degree (c: Cell) =
                    fl.Adjacency
                    |> Array.sumBy (fun (a, b) -> (if a = c then 1 else 0) + (if b = c then 1 else 0))

                let specials =
                    fl.Rooms |> Array.filter (fun rm -> rm.Kind <> Normal && rm.Kind <> Start)
            ...
            <| fun () ->
                let gen (s: uint64) =
                    let struct (m, _) = MapGen.maze 25 17 (Rng.ofSeed s) in m
            ...
            <| fun () ->
                ...
                let allFalse: Grid<bool> =
                    {
                        Width = w
                        Height = h
                        Cells = Array.create (w * h) false
                    }
                    ...
                    let mask: Grid<bool> =
                        {
                            Width = max 0 ww
                            Height = max 0 hh
                            Cells = Array.create (max 0 ww * max 0 hh) true
                        }
