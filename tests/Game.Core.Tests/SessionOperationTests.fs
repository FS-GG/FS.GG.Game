...
[<Tests>]
let tests =
    testList "Game.Core portable session operations" [
        test "configuration enforces a bounded required-record window" {
            ...
            let refused, effects =
                SessionOperations.update
                    (SessionOperationObservation.EnqueueRequired "second")
                    state
            Expect.equal refused state "capacity refusal preserves state"
            Expect.equal effects [ SessionOperationEffect.Refused(SessionOperationRefusal.RequiredCapacityReached 1u) ] "capacity is explicit"
        }
        ...
        test "projection demand coalesces behind one request and revisions stay monotonic" {
            ...
            Expect.equal refreshed.PendingProjection (Some second.Id.Operation) "the refresh is the sole pending projection"

            let stale, staleEffects =
                SessionOperations.update
                    (SessionOperationObservation.CompleteProjection(second.Id, 7UL, "duplicate-revision"))
                    refreshed
            Expect.equal stale.LastProjectionRevision (Some 7UL) "a stale projection cannot replace the accepted revision"
            Expect.equal staleEffects
        ...
        test "replacement cancellation and failure invalidate stale generations" {
            ...
            let failure = { Code = "server-unavailable"; Message = "try another generation" }
            let failed, _ =
                SessionOperations.update
                    (SessionOperationObservation.Fail(failing.Id, failure))
                    active
            ...
            let cancelled, _ = SessionOperations.update SessionOperationObservation.Cancel failed
            Expect.equal cancelled.Generation 3UL "cancellation invalidates the failed generation"
            Expect.equal cancelled.Status SessionOperationStatus.Cancelled "cancellation is terminal until replacement"
            Expect.equal
            ...
            Expect.equal lateEffects
                [ SessionOperationEffect.Refused(
                    SessionOperationRefusal.StaleGeneration(cancelledProjection.Generation, lateProjection.Id.Generation)) ]
        ...
        test "disposal is idempotent and cannot be replaced" {
            ...
            Expect.equal refused
                [ SessionOperationEffect.Refused(SessionOperationRefusal.CoordinatorInactive SessionOperationStatus.Disposed) ]
