...
let tests =
    testList
        "Game.Core Geometry (US1, FR-001..FR-005)"
        [

            testList
                "intersects (strict edges)"
                [
                    ...
                    test "edge-touching rectangles do NOT intersect (strict)" {
                        Expect.isFalse
                            (Geometry.intersects (r 0. 0. 10. 10.) (r 10. 0. 10. 10.))
                            "shared edge is zero-area ⇒ false"
                    }
                    test "corner-touching rectangles do NOT intersect (strict)" {
                        Expect.isFalse
                            (Geometry.intersects (r 0. 0. 10. 10.) (r 10. 10. 10. 10.))
                            "shared corner ⇒ false"
                    }
                    testCase "intersects is symmetric (FsCheck ≥500 cases)"
                    <| fun () ->
            ...
            testList
                "contains / containsPoint (inclusive edges)"
                [
                    ...
                    test "inner flush against the outer edge is contained (inclusive)" {
                        Expect.isTrue
                            (Geometry.contains (r 0. 0. 20. 20.) (r 0. 0. 20. 20.))
                            "flush/coincident ⇒ contained"
                    }
                    test "inner poking outside ⇒ false" {
                        Expect.isFalse
                            (Geometry.contains (r 0. 0. 20. 20.) (r 15. 15. 10. 10.))
                            "spills past edge ⇒ false"
                    }
            ...
            testList
                ...
                [
                    ...
                    testCase "center (ofCenter c w h) = c round-trip (FsCheck ≥500 cases)"
                    <| fun () ->
            ...
            testList
                ...
                [
                    test "a fast projectile tunneling through a thin target is detected" {
                        ...
                        Expect.isTrue
                            (Geometry.sweptIntersects bullet (p 100. 0.) wall)
                            "the swept path crosses the wall ⇒ true"
                    }
                    test "no motion + no overlap ⇒ false" {
                        Expect.isFalse
                            (Geometry.sweptIntersects (r 0. 0. 2. 2.) (p 0. 0.) (r 50. 0. 1. 10.))
                            "still & apart ⇒ false"
                    }
                    testCase "sweptIntersects is a superset of intersects at both endpoints (FsCheck ≥500)"
                    <| fun () ->
            ...
            testList
                "aabbContact (narrow-phase manifold)"
                [

                    let isUnitAxis (n: Point) =
                        (abs n.X = 1.0 && n.Y = 0.0) || (n.X = 0.0 && abs n.Y = 1.0)

                    testCase "isSome agrees with intersects, always (FsCheck ≥500)"
                    <| fun () ->
                    ...
                    testCase "a contact has positive depth and a unit-axis normal (FsCheck ≥500)"
                    <| fun () ->
                        let prop a b c d e f g h =
                    ...
                    testCase "penetration depth is symmetric in the operands (FsCheck ≥500)"
                    <| fun () ->
                        let prop a b c d e f g h =
                            let x = rectOf a b c d
                            let y = rectOf e f g h
                    ...
                    testCase "translating by the MTV separates the pair (FsCheck ≥500)"
                    <| fun () ->
                    ...
                    test "normal is anti-symmetric: contact a→b negates contact b→a" {
                        ...
                        Expect.equal
                            (Geometry.aabbContact b a)
                            (Some { Normal = p -1. 0.; Depth = 2. })
                            "b→a points -x, same depth"
                    }
                    ...
                    testList
                        "determinism golden (byte-identical manifolds)"
                        [
                            test "least-penetration axis is X (shallow horizontal overlap)" {
                                Expect.equal
                                    (Geometry.aabbContact (r 0. 0. 10. 10.) (r 8. 0. 10. 10.))
                                    (Some { Normal = p 1. 0.; Depth = 2. })
                                    "X axis, +x, depth 2"
                            }
                            test "least-penetration axis is Y (shallow vertical overlap)" {
                                Expect.equal
                                    (Geometry.aabbContact (r 0. 0. 10. 10.) (r 0. 8. 10. 10.))
                                    (Some { Normal = p 0. 1.; Depth = 2. })
                                    "Y axis, +y, depth 2"
                            }
                            ...
                            test "equal penetration on both axes breaks toward X (documented tie-break)" {
                                Expect.equal
                                    (Geometry.aabbContact (r 0. 0. 10. 10.) (r 8. 8. 10. 10.))
                                    (Some { Normal = p 1. 0.; Depth = 2. })
                                    "px = py ⇒ X axis"
                            }
                            ...
                            test "full containment still yields a deterministic MTV" {
                                Expect.equal
                                    (Geometry.aabbContact (r 0. 0. 10. 10.) (r 3. 3. 2. 2.))
                                    (Some { Normal = p -1. 0.; Depth = 5. })
                                    "inner box: X axis tie-break, depth 5"
                            }
                        ]
                ]
            ...
            testList
                "circleContact / circleAabbContact (narrow-phase)"
                [

                    let circ x y rad : Circle =
                        {
                            Center = { X = x; Y = y }
                            Radius = rad
                        }
                    ...
                    let circleOf (a: int) (b: int) (rr: int) : Circle =
                        {
                            ...
                            Radius = float (1 + abs (rr % 20))
                        }

                    let isUnit (n: Point) =
                        abs (sqrt (n.X * n.X + n.Y * n.Y) - 1.0) < 1e-9

                    testCase "circleContact isSome agrees with squared-distance overlap (FsCheck ≥500)"
                    <| fun () ->
                    ...
                    testCase "circleContact manifold has positive depth and a unit normal (FsCheck ≥500)"
                    <| fun () ->
                        let prop a b ra c d rb =
                    ...
                    testCase "circleContact MTV separates the pair within tolerance (FsCheck ≥500)"
                    <| fun () ->
                        let prop a b ra c d rb =
                            let x = circleOf a b ra
                            let y = circleOf c d rb
                            ...
                            | Some k ->
                                let x' =
                                    { x with
                                        Center =
                                            {
                                                X = x.Center.X - k.Normal.X * k.Depth
                                                Y = x.Center.Y - k.Normal.Y * k.Depth
                                            }
                                    }
                    ...
                    testCase "circleAabbContact isSome agrees with clamp-distance overlap (FsCheck ≥500)"
                    <| fun () ->
                    ...
                    testCase "circleAabbContact MTV separates within tolerance (FsCheck ≥500)"
                    <| fun () ->
                        let prop a b ra e f g h =
                            let c = circleOf a b ra
                            let box = rectOf e f g h
                            ...
                            | Some k ->
                                let c' =
                                    { c with
                                        Center =
                                            {
                                                X = c.Center.X - k.Normal.X * k.Depth
                                                Y = c.Center.Y - k.Normal.Y * k.Depth
                                            }
                                    }
                    ...
                    test "NaN / non-positive radius return None, never throw (total)" {
                        ...
                        Expect.isNone
                            (Geometry.circleAabbContact (circ nan 0. 2.) (r 0. 0. 10. 10.))
                            "NaN centre ⇒ None"

                        Expect.isNone
                            (Geometry.circleAabbContact (circ 5. 5. 0.) (r 0. 0. 10. 10.))
                            "zero radius ⇒ None"
                    }
                    ...
                    testList
                        "determinism golden (circle manifolds)"
                        [
                            test "circle-circle shallow overlap on X" {
                                Expect.equal
                                    (Geometry.circleContact (circ 0. 0. 3.) (circ 4. 0. 2.))
                                    (Some { Normal = p 1. 0.; Depth = 1. })
                                    "normal +x, depth (5-4)=1"
                            }
                            test "circle-circle coincident centres use the fixed fallback normal" {
                                Expect.equal
                                    (Geometry.circleContact (circ 2. 2. 3.) (circ 2. 2. 1.))
                                    (Some { Normal = p 1. 0.; Depth = 4. })
                                    "coincident ⇒ (1,0), depth rA+rB"
                            }
                            test "circle-circle touching is not a contact (strict)" {
                                Expect.isNone
                                    (Geometry.circleContact (circ 0. 0. 2.) (circ 4. 0. 2.))
                                    "d = rA+rB ⇒ None"
                            }
                            ...
                            test "circle-AABB centre outside: normal points circle to box" {
                                Expect.equal
                                    (Geometry.circleAabbContact (circ 13. 5. 4.) (r 0. 0. 10. 10.))
                                    (Some { Normal = p -1. 0.; Depth = 1. })
                                    "right of box ⇒ normal -x, depth (4-3)=1"
                            }
                            test "circle-AABB centre at box centre: least-penetration X face, +bias" {
                                Expect.equal
                                    (Geometry.circleAabbContact (circ 5. 5. 2.) (r 0. 0. 10. 10.))
                                    (Some { Normal = p -1. 0.; Depth = 7. })
                                    "tie ⇒ X axis, depth pen(5)+r(2)"
                            }
                            test "circle-AABB centre near left face: escape left, normal inward" {
                                Expect.equal
                                    (Geometry.circleAabbContact (circ 1. 5. 2.) (r 0. 0. 10. 10.))
                                    (Some { Normal = p 1. 0.; Depth = 3. })
                                    "nearest left face ⇒ normal +x, depth pen(1)+r(2)"
                            }
                            test "circle-AABB gap is not a contact" {
                                Expect.isNone
                                    (Geometry.circleAabbContact (circ 20. 20. 2.) (r 0. 0. 10. 10.))
                                    "far ⇒ None"
                            }
                        ]
                ]
            ...
            testList
                "segmentAabbHit / segmentCircleHit (raycast)"
                [

                    let circR x y rad : Circle =
                        {
                            Center = { X = x; Y = y }
                            Radius = rad
                        }
                    ...
                    testCase "segmentAabbHit: t in [0,1], point on the box boundary, unit-axis normal (FsCheck ≥500)"
                    <| fun () ->
                        let prop a b c d e f g h =
                            let p0 =
                                {
                            ...
                            | Some hit ->
                                let onEdge =
                                    abs (hit.Point.X - box.X) < 1e-9
                                    || abs (hit.Point.X - (box.X + box.Width)) < 1e-9
                                    || abs (hit.Point.Y - box.Y) < 1e-9
                                    || abs (hit.Point.Y - (box.Y + box.Height)) < 1e-9

                                let unitAxis =
                                    (abs hit.Normal.X = 1.0 && hit.Normal.Y = 0.0)
                                    || (hit.Normal.X = 0.0 && abs hit.Normal.Y = 1.0)
                    ...
                    testCase "segmentCircleHit: t in [0,1], point on the circle, unit normal (FsCheck ≥500)"
                    <| fun () ->
                    ...
                    test "NaN / zero-length / non-positive radius return None, never throw (total)" {
                        Expect.isNone
                            (Geometry.segmentAabbHit (p nan 0.) (p 5. 5.) (r 0. 0. 10. 10.))
                            "NaN origin ⇒ None"

                        Expect.isNone
                            (Geometry.segmentAabbHit (p 5. 5.) (p 5. 5.) (r 0. 0. 10. 10.))
                            "zero-length inside ⇒ None"

                        Expect.isNone
                            (Geometry.segmentCircleHit (p nan 0.) (p 5. 0.) (circR 0. 0. 2.))
                            "NaN origin ⇒ None"

                        Expect.isNone
                            (Geometry.segmentCircleHit (p 0. 0.) (p 0. 0.) (circR 0. 0. 2.))
                            "zero-length ⇒ None"

                        Expect.isNone
                            (Geometry.segmentCircleHit (p -5. 0.) (p 5. 0.) (circR 0. 0. 0.))
                            "non-positive radius ⇒ None"
                    }

                    // Determinism golden — fixed inputs, exact expected hits (byte-identical). Named
                    // "determinism golden" so the gate.yml zero-match guard's determinism filter covers them.
                    testList
                        "determinism golden (raycast)"
                        [
                            test "segment enters an AABB through the left face" {
                                Expect.equal
                                    ...
                                    "enter left ⇒ t 0.5, point (0,5), normal -x"
                            }
                            ...
                            test "segment starting inside the AABB has no entry" {
                                Expect.isNone
                                    (Geometry.segmentAabbHit (p 5. 5.) (p 20. 5.) (r 0. 0. 10. 10.))
                                    "origin inside ⇒ None"
                            }
                            test "segment missing the AABB" {
                                Expect.isNone
                                    (Geometry.segmentAabbHit (p -5. 20.) (p 5. 20.) (r 0. 0. 10. 10.))
                                    "above box ⇒ None"
                            }
                            test "segment enters a circle at the near root" {
                                Expect.equal
                                    (Geometry.segmentCircleHit (p -4. 0.) (p 4. 0.) (circR 0. 0. 2.))
                                    (Some
                                        {
                                            T = 0.25
                                            Point = p -2. 0.
                                    ...
                                    "near root ⇒ t 0.25, point (-2,0), normal -x"
                            }
                            test "segment starting inside the circle has no entry" {
                                Expect.isNone
                                    (Geometry.segmentCircleHit (p 0. 0.) (p 10. 0.) (circR 0. 0. 2.))
                                    "origin at centre ⇒ None"
                            }
                            test "segment missing the circle" {
                                Expect.isNone
                                    (Geometry.segmentCircleHit (p -5. 5.) (p 5. 5.) (circR 0. 0. 2.))
                                    "line y=5 misses r=2 ⇒ None"
                            }
                        ]
                ]
            ...
            testList
                "obbPolygon / polygonContact (SAT convex)"
                [
                    ...
                    testCase "isSome agrees with aabbContact for rotation-0 OBBs (FsCheck ≥500)"
                    <| fun () ->
                        ...
                        let prop a b c d e f g h =
                            ...
                            (Geometry.polygonContact pa pb |> Option.isSome) =
                                (Geometry.aabbContact ra rb |> Option.isSome)

                        Check.One(Config.QuickThrowOnFailure.WithMaxTest 500, prop)

                    testCase "a contact has positive depth and a unit normal (FsCheck ≥500)"
                    <| fun () ->
                    ...
                    testCase "translating by the MTV separates the pair within tolerance (FsCheck ≥500)"
                    <| fun () ->
                                ...
                                let x' =
                                    {
                                        Vertices =
                                            ...
                                            |> Array.map (fun v ->
                                                {
                                                    X = v.X - k.Normal.X * k.Depth
                                                    Y = v.Y - k.Normal.Y * k.Depth
                                                })
                                    }
                    ...
                    test "obbPolygon at rotation 0 is the axis-aligned box, CCW (FR-003)" {
                        Expect.equal
                            (Geometry.obbPolygon (p 0. 0.) (p 1. 1.) 0.0)
                            {
                                Vertices = [| p -1. -1.; p 1. -1.; p 1. 1.; p -1. 1. |]
                            }
                            "CCW corners of the unit box"
                    }
                    ...
                    test "two rotated OBBs that overlap produce a contact" {
                        ...
                        Expect.isTrue
                            (Geometry.polygonContact (obb 0. 0. 1. 1.) diamond |> Option.isSome)
                            "rotated overlap ⇒ Some"
                    }

                    test "degenerate inputs return None, never throw (total)" {
                        Expect.isNone
                            (Geometry.polygonContact { Vertices = [| p 0. 0.; p 1. 0. |] } (obb 0. 0. 1. 1.))
                            "<3 vertices ⇒ None"

                        Expect.isNone
                            (Geometry.polygonContact
                                {
                                    Vertices = [| p nan 0.; p 1. 0.; p 0. 1. |]
                                }
                                (obb 0. 0. 1. 1.))
                            "NaN coordinate ⇒ None"
                        ...
                        Expect.isNone
                            (Geometry.polygonContact
                                {
                                    Vertices = [| p 0. 0.; p 1. 0.; p 2. 0. |]
                                }
                                (obb 0. 0. 1. 1.))
                            "collinear/zero-area ⇒ None"
                    }

                    // Determinism golden — fixed inputs, exact expected manifolds (byte-identical). Named
                    // "determinism golden" so the gate.yml zero-match guard's determinism filter covers them.
                    testList
                        "determinism golden (SAT convex manifolds)"
                        [
                            test "shallow X overlap ⇒ least-overlap axis X, normal a→b +x" {
                                Expect.equal
                                    (Geometry.polygonContact (obb 0. 0. 1. 1.) (obb 1.5 0. 1. 1.))
                                    (Some { Normal = p 1. 0.; Depth = 0.5 })
                                    "X axis, +x, depth 0.5"
                            }
                            test "equal X/Y overlap ⇒ first axis in generation order (a edge0 = Y face)" {
                                ...
                                Expect.equal
                                    (Geometry.polygonContact (obb 0. 0. 1. 1.) (obb 1.5 1.5 1. 1.))
                                    (Some { Normal = p 0. 1.; Depth = 0.5 })
                                    "tie ⇒ Y axis (generation order), +y, depth 0.5"
                            }
                            test "touching squares are not a contact (strict edges)" {
                                Expect.isNone
                                    (Geometry.polygonContact (obb 0. 0. 1. 1.) (obb 2. 0. 1. 1.))
                                    "shared edge ⇒ zero overlap ⇒ None"
                            }
                            ...
                            test "full containment yields the true exit depth (containment correction)" {
                                ...
                                Expect.equal
                                    (Geometry.polygonContact (obb 0. 0. 5. 5.) (obb 0. 3. 1. 0.5))
                                    (Some { Normal = p 0. 1.; Depth = 2.5 })
                                    "contained box escapes +y, depth 2.5"
                            }
                        ]
                ]
            ...
            testList
                "polygonManifold (contact points, pair ids, feature ids)"
                [
                    ...
                    let distToBoundary (poly: ConvexPolygon) (q: Point) =
                        ...
                        for i in 0 .. v.Length - 1 do
                            ...
                            let t =
                                if len2 > 0.0 then
                                    max 0.0 (min 1.0 (((q.X - s.X) * ex + (q.Y - s.Y) * ey) / len2))
                                else
                                    0.0
                    ...
                    testCase "isSome agrees with polygonContact for all inputs (FsCheck ≥500)"
                    <| fun () ->
                        let prop a b c d e f g h i j =
                    ...
                    testCase "Normal and Depth agree with polygonContact bit for bit (FsCheck ≥500)"
                    <| fun () ->
                    ...
                    testCase "rotation-0 OBBs: Normal and Depth agree with aabbContact (FsCheck ≥500)"
                    <| fun () ->
                    ...
                    testCase "contact points lie on the boundary of both shapes within tolerance (FsCheck ≥500)"
                    <| fun () ->
                    ...
                    testCase "shape invariants: A<B, 1-2 points, PointCount=Points.Length, unit normal (FsCheck ≥500)"
                    <| fun () ->
                        let prop a b c d e f g h i j =
                    ...
                    testCase "the manifold is a pure function of the pair (FsCheck ≥500)"
                    <| fun () ->
                    ...
                    testCase
                        "the manifold rides a common translation: ids/normal/depth exactly, points to a few ULP (FsCheck ≥500)"
                    <| fun () ->
                    ...
                    testCase "#87: the translated manifold keeps every exact claim, and its points to a few ULP"
                    <| fun () ->
                    ...
                    testCase "arbitrary convex rings: points on the boundary of both shapes (FsCheck ≥500)"
                    <| fun () ->
                        // Andrew's monotone chain; the `<= 0.0` pop drops collinear points, so the ring is strict.
                        let hull (pts: Point[]) : Point[] =
                            let ps =
                                pts |> Array.distinctBy (fun q -> q.X, q.Y) |> Array.sortBy (fun q -> q.X, q.Y)
                        ...
                        let distToBoundary (poly: ConvexPolygon) (q: Point) =
                            ...
                            Seq.init v.Length (fun i ->
                                ...
                                let t =
                                    max 0.0 (min 1.0 (((q.X - s.X) * ex + (q.Y - s.Y) * ey) / (ex * ex + ey * ey)))
                        ...
                        let prop (seeds: int[]) (tx: int) (ty: int) =
                            ...
                            let pt (i: int) =
                                p (float (i % 17) * 0.37) (float ((i / 17) % 17) * 0.41)
                            ...
                            if seeds.Length < 6 then
                                ...
                                let hb =
                                    hull (
                                        seeds
                                        |> Array.rev
                                        |> Array.map (fun i -> let q = pt (i + 3) in p (q.X + dx) (q.Y + dy))
                                    )
                    ...
                    test "degenerate inputs return ValueNone, never throw (total)" {
                        Expect.isTrue
                            (Geometry.polygonManifold { Vertices = [| p 0. 0.; p 1. 0. |] } (obb 0. 0. 1. 1.)
                             |> ValueOption.isNone)
                            "<3 vertices ⇒ ValueNone"

                        Expect.isTrue
                            (Geometry.polygonManifold
                                {
                                    Vertices = [| p nan 0.; p 1. 0.; p 0. 1. |]
                                }
                                (obb 0. 0. 1. 1.)
                    ...
                    // "determinism golden" so the gate.yml zero-match guard's determinism filter covers them.
                    testList
                        "determinism golden (contact points and feature ids)"
                        [
                            test "face-on-face ⇒ two points on the shared face, feature id (refEdge 1, incEdge 3)" {
                                ...
                                Expect.equal
                                    (Geometry.polygonManifold (obb 0. 0. 1. 1.) (obb 1.5 0. 1. 1.))
                                    (ValueSome
                                        {
                                            A = 0
                                            B = 1
                            ...
                            test "vertex-into-face ⇒ one point, the poking vertex" {
                                ...
                                | ValueSome m ->
                                    ...
                                    Expect.equal
                                        m.FeatureId
                                        ((1 <<< 15) ||| 2)
                                        "a's right face (1), the diamond's upper-left face (2)"
                            }
                            test "the reference face flips to b when only b has a face on the axis" {
                                ...
                                | ValueSome m ->
                                    Expect.equal
                                        m.FeatureId
                                        ((1 <<< 30) ||| (1 <<< 15) ||| 2)
                                        "flip bit, refEdge 1 (b), incEdge 2 (a)"
                                    ...
                                    Expect.equal
                                        m.PointCount
                                        1
                                        "the same single vertex contact, seen from the other side"
                            ...
                            test "touching squares are not a contact (strict edges)" {
                                Expect.isTrue
                                    (Geometry.polygonManifold (obb 0. 0. 1. 1.) (obb 2. 0. 1. 1.)
                                     |> ValueOption.isNone)
            ...
            testList
                "segmentPolygonHit (segment vs convex polygon)"
                [
                    ...
                    testCase "agrees with segmentAabbHit on rotation-0 OBBs, but for the corner graze (FsCheck ≥500)"
                    <| fun () ->
                    ...
                    testCase
                        "a hit has t in [0,1], a point on the boundary, and an inward-facing unit normal (FsCheck ≥500)"
                    <| fun () ->
                        ...
                        let prop a b c d e f g h i =
                            ...
                            | Some hit ->
                                ...
                                let onSomeEdgeLine =
                                    poly.Vertices
                                    |> Array.exists (fun v ->
                                        abs ((hit.Point.X - v.X) * hit.Normal.X + (hit.Point.Y - v.Y) * hit.Normal.Y) <
                                            1e-9 * (1.0 + segLen))
                    ...
                    testCase "a transversal entry parameter is invariant under rotating the whole scene (FsCheck ≥500)"
                    <| fun () ->
                        ...
                        let prop a b c d e f g h i =
                            ...
                            if not (signedDist flatPoly p0 > 1e-3 && signedDist flatPoly p1 < -1e-3) then
                                ...
                                match
                                    Geometry.segmentPolygonHit p0 p1 flatPoly,
                                    Geometry.segmentPolygonHit (rot p0) (rot p1) spunPoly
                                with
                    ...
                    test "degenerate inputs return None, never throw (total)" {
                        let tri: ConvexPolygon =
                            {
                                Vertices = [| p 0. 0.; p 4. 0.; p 0. 4. |]
                            }

                        Expect.isNone
                            (Geometry.segmentPolygonHit (p 2. -2.) (p 2. 2.) { Vertices = [| p 0. 0.; p 1. 0. |] })
                            "<3 vertices ⇒ None"

                        Expect.isNone
                            (Geometry.segmentPolygonHit
                                (p 2. -2.)
                                (p 2. 2.)
                                {
                                    Vertices = [| p 0. 0.; p 1. 0.; p 2. 0. |]
                        ...
                        Expect.isNone
                            (Geometry.segmentPolygonHit (p 2. -2.) (p 2. -2.) tri)
                            "zero-length outside ⇒ None"
                    ...
                    // "determinism golden" so the gate.yml zero-match guard's determinism filter covers them.
                    testList
                        "determinism golden (polygon raycast)"
                        [
                            test "segment enters a rotation-0 OBB through the left edge" {
                                Expect.equal
                                    (Geometry.segmentPolygonHit (p -5. 5.) (p 5. 5.) box10)
                                    (Some
                                        {
                                            T = 0.5
                                            Point = p 0. 5.
                            ...
                            test "segment enters an arbitrary convex polygon through the edge it crosses" {
                                ...
                                Expect.equal
                                    ...
                                    "enter bottom ⇒ t 0.5, point (2,0), normal -y"
                            }
                            test "segment entering at a corner resolves to the first edge in vertex order (tie-break)" {
                                ...
                                Expect.equal
                                    ...
                                    "…where the AABB cast ties to the X face"
                            }
                            test "corner tie-break holds on a rotated ring, where the parameters tie only to an ULP" {
                                ...
                                let ring: ConvexPolygon =
                                    {
                                        Vertices = [| p -4. -2.; p -3. -3.; p -1. -4.; p 2. -4.; p -1. 3. |]
                                    }
                            ...
                            test "a segment collinear with an edge is a hit (its chord has positive length)" {
                                ...
                                Expect.equal
                                    (Geometry.segmentPolygonHit (p -5. 0.) (p 15. 0.) box10)
                                    (Some
                                        {
                                            T = 0.25
                                            Point = p 0. 0.
                                    ...
                                    "along the bottom edge ⇒ enter left, t 0.25"

                                Expect.equal
                                    (Geometry.segmentAabbHit (p -5. 0.) (p 15. 0.) (r 0. 0. 10. 10.))
                                    (Some
                                        {
                                            T = 0.25
                                            Point = p 0. 0.
                                    ...
                                    "…and the AABB cast agrees exactly"
                                // Starting ON the boundary and staying on it never enters: no edge is crossed inward.
                                Expect.isNone
                                    (Geometry.segmentPolygonHit (p 2. 0.) (p 8. 0.) box10)
                                    "starts on the edge ⇒ None"

                                Expect.isNone
                                    (Geometry.segmentAabbHit (p 2. 0.) (p 8. 0.) (r 0. 0. 10. 10.))
                                    "…as does the AABB cast"
                            }
                            test "a segment grazing one corner is not a hit (strict edges)" {
                                ...
                                Expect.isSome
                                    (Geometry.segmentAabbHit (p -5. 5.) (p 5. -5.) (r 0. 0. 10. 10.))
                                    "…where the AABB cast reports a hit"
                            }
                            test "a graze is a knife-edge: rotating the scene can turn it into a hit" {
                                ...
                                Expect.isNone
                                    (Geometry.segmentPolygonHit p0 p1 (Geometry.obbPolygon (p 0. -2.) (p 2. 2.) 0.0))
                                ...
                                match
                                    Geometry.segmentPolygonHit
                                        (rot p0)
                                        (rot p1)
                                        (Geometry.obbPolygon (rot (p 0. -2.)) (p 2. 2.) theta)
                                with
                                | Some hit ->
                                    Expect.floatClose
                                        Accuracy.high
                                        hit.T
                                        (1.0 / 3.0)
                                        "the rotated cast enters at the same T"

                                    Expect.floatClose
                                        Accuracy.high
                                        hit.Point.X
                                        (rot (p 2. -4.)).X
                                        "…at the rotated corner"
                            ...
                            test "a boundary-tangent segment is a knife-edge: rotating the scene moves T" {
                                ...
                                | Some hit ->
                                    Expect.equal
                                        hit.T
                                        (2.0 / 3.0)
                                        "flat: enters at the corner it reaches along the edge line"
                                    ...
                                    Expect.equal
                                        hit.Normal
                                        (p 0. -1.)
                                        "flat: the bottom edge it crossed to reach the boundary"
                                ...
                                match
                                    Geometry.segmentPolygonHit
                                        (rot p0)
                                        (rot p1)
                                        (Geometry.obbPolygon (rot centre) half theta)
                                with
                                | Some hit ->
                                    Expect.floatClose
                                        Accuracy.high
                                        hit.T
                                        1.0
                                        "rotated: nudged outside, touches only at its endpoint"
                            ...
                            test "segment starting inside the polygon has no entry" {
                                Expect.isNone
                                    (Geometry.segmentPolygonHit (p 5. 5.) (p 20. 5.) box10)
                                    "origin inside ⇒ None"
                            }
                            test "segment missing the polygon" {
                                Expect.isNone
                                    (Geometry.segmentPolygonHit (p -5. 20.) (p 5. 20.) box10)
                                    "above the box ⇒ None"
                            }
                            test "polygon behind the segment is not a hit" {
                                Expect.isNone
                                    (Geometry.segmentPolygonHit (p -20. 5.) (p -15. 5.) box10)
                                    "box beyond p1 ⇒ None"
                            }
                            test "segment enters a rotated OBB through the rotated edge it crosses" {
                                ...
                                | Some hit ->
                                    ...
                                    Expect.floatClose
                                        ...
                                        hit.Normal.X
                                        (-(sqrt 0.5))
                                        "outward normal x = -1/√2"

                                    Expect.floatClose
                                        Accuracy.high
                                        hit.Normal.Y
                                        (-(sqrt 0.5))
                                        "outward normal y = -1/√2"
