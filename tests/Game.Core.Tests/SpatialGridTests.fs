...
let private goodCell (cs: float) =
    if System.Double.IsNaN cs || System.Double.IsInfinity cs || cs <= 0.0 then
        1.0
    else
        min 1.0e4 (abs cs + 0.5)

// Brute-force oracles the grid must match exactly.
let private bruteRect (region: Rect) (items: (Point * 'T) list) =
    items
    |> List.filter (fun (p, _) -> Geometry.containsPoint region p)
    |> List.map snd
...
let private bruteBounds (region: Rect) (items: (Rect * 'T) list) =
    items
    |> List.filter (fun (r, _) -> Geometry.intersects region r)
    |> List.map snd
...
let tests =
    testList
        "Game.Core SpatialGrid (US2, FR-006..FR-009)"
        [
            ...
            test "radius query returns items within distance, squared-distance boundary inclusive" {
                let items =
                    [ pt 10.0 11.0, "a"; pt 20.0 20.0, "b"; pt 12.0 9.0, "c"; pt 13.0 10.0, "d" ]
            ...
            test "repeat builds and queries are byte-identical (determinism)" {
                ...
                Expect.equal
                    (SpatialGrid.queryRadius (pt 2.0 2.0) 2.5 g1)
                    (SpatialGrid.queryRadius (pt 2.0 2.0) 2.5 g2)
                    "same radius result"
            }
            ...
            test "huge (out-of-range) coordinates stay exact via the O(n) fallback" {
                ...
                let items =
                    [ pt 1.0e15 1.0e15, "far"; pt 2.0 2.0, "near"; pt 1.0e15 1.0e15, "far2" ]
                ...
                Expect.equal
                    ...
                    [ "far"; "far2" ]
                    "exact hit on huge coords, insertion order"

                Expect.equal
                    (SpatialGrid.queryRadius (pt 1.0e15 1.0e15) 1.0 grid)
                    [ "far"; "far2" ]
                    "radius exact on huge coords"
            }

            testCase "rect query equals brute-force filter, in insertion order (FsCheck)"
            <| fun () ->
            ...
            testCase "radius query equals brute-force filter, in insertion order (FsCheck)"
            <| fun () ->
            ...
            test "a large item is found from a query that its ORIGIN sits far outside" {
                ...
                let items =
                    fillers
                    @ [ rect -100.0 -100.0 200.0 200.0, "big"; rect 89.0 89.0 2.0 2.0, "small" ]

                let grid = SpatialGrid.buildBounds 8.0 items

                Expect.equal
                    (SpatialGrid.queryBounds (rect 89.5 89.5 1.0 1.0) grid)
                    [ "big"; "small" ]
                    "insertion order"
            }
            ...
            test "an item too large to bucket stays exact, and costs only itself" {
                ...
                let items =
                    fillers
                    @ [ rect -500000.0 -1.0 1000000.0 2.0, "floor"; rect 0.0 0.5 1.0 1.0, "crate" ]

                let grid = SpatialGrid.buildBounds 0.5 items

                Expect.equal
                    (SpatialGrid.queryBounds (rect 0.1 0.6 0.5 0.5) grid)
                    [ "floor"; "crate" ]
                    "deferred item still found"

                Expect.equal
                    (SpatialGrid.queryBounds (rect 0.1 50.0 0.5 0.5) grid)
                    []
                    "and not found where it does not reach"
            }

            test "non-finite bounds and a non-finite region stay total and exact" {
                let items =
                    fillers
                    @ [ rect 0.0 0.0 infinity infinity, "inf"; rect 1.0 1.0 1.0 1.0, "unit" ]

                let grid = SpatialGrid.buildBounds 1.0 items

                Expect.equal
                    (SpatialGrid.queryBounds (rect 1.2 1.2 0.5 0.5) grid)
                    [ "inf"; "unit" ]
                    "unbounded item overlaps"
            ...
            test "non-positive cellSize falls back to a single bucket but stays exact (bounds grid)" {
                let items =
                    [
                        ...
                        rect 90.0 90.0 2.0 2.0, "b"
                        rect 1.0 1.0 2.0 2.0, "c"
                    ]
            ...
            testCase "bounds query equals brute-force intersects, in insertion order (FsCheck)"
            <| fun () ->
                ...
                let prop (boxes: (float * float * float * float) list) (cs: float) (r: float * float * float * float) =
                    let items =
                        boxes
                        |> List.mapi (fun i (x, y, w, h) -> rect (clamp x) (clamp y) (clamp w) (clamp h), i)
