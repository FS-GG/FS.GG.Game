...
let tests =
    testList
        "Game.Core Dice (023, FR-001..FR-006)"
        [
            ...
            test "advantage/disadvantage are max/min with monotone means (FR-003)" {
                ...
                Expect.isTrue
                    (near (Dice.mean adv + Dice.mean dis) (2.0 * Dice.mean d))
                    "adv + dis means sum to 2× single (max+min=a+b)"
            }
            ...
            test "mean does not overflow at high dice counts (bigint numerator regression, FR-004)" {
                ...
                Expect.isTrue
                    (near (Dice.mean (Dice.repeat 22 (Dice.die 6))) 77.0)
                    "mean of 22d6 is 77 (no int64 overflow)"
            }

            testCase "convolve mean equals the sum of means over random dice (FsCheck)"
            <| fun () ->
            ...
            testCase "convolve is commutative and associative over random dice (FsCheck)"
            <| fun () ->
                let prop (x: int) (y: int) (z: int) =
                    ...
                    && Dice.outcomes (Dice.convolve (Dice.convolve a b) c) =
                        Dice.outcomes (Dice.convolve a (Dice.convolve b c))
