...
let private cellOf (col: int) (row: int) =
    {
        Col = ((col % 8) + 8) % 8
        Row = ((row % 8) + 8) % 8
    }
...
let tests =
    testList
        "Game.Core Edges (020, FR-001..FR-006)"
        [
            ...
            testCase "edgeBetween order-independence + edgeOf round-trip over random adjacent pairs (FsCheck)"
            <| fun () ->
            ...
            testCase "relationships are mutually consistent (FR-003, FsCheck)"
            <| fun () ->
                let prop (col: int) (row: int) =
                    ...
                    let cornersOk =
                        Edges.corners c |> List.forall (fun v -> Edges.vertexCells v |> List.contains c)
            ...
            testCase "empty walls match plain Pathfinding.bfs reachability + hop count (FR-005, FsCheck)"
            <| fun () ->
            ...
            testCase "determinism: relationships and edge-aware search are byte-identical (FR-006, FsCheck)"
            <| fun () ->
                let prop
                    (blockedRaw: (int * int) list)
                    (wallsRaw: (int * int) list)
                    (sc: int)
                    (sr: int)
                    (gc: int)
                    (gr: int)
