...
let tests =
    testList
        "Game.Core Resolution (response layer, FR-001..FR-005)"
        [

            testList
                "pushOut (separate along the MTV)"
                [

                    testCase "pushing a body out by its aabbContact manifold removes the overlap (FsCheck ≥500)"
                    <| fun () ->
                    ...
                    test "a zero-depth contact leaves the position unchanged" {
                        Expect.equal
                            (Resolution.pushOut (p 5. 5.) { Normal = p 1. 0.; Depth = 0. })
                            (p 5. 5.)
                            "zero depth ⇒ identity"
                    }
            ...
            testList
                "slide (kill normal, keep tangential)"
                [

                    testCase "result has zero normal component and preserves the tangential (FsCheck ≥500)"
                    <| fun () ->
            ...
            testList
                "knockback (discrete grid, stop before a blocker)"
                [

                    testCase "never lands on a blocked cell and never exceeds distance (FsCheck ≥500)"
                    <| fun () ->
                        ...
                        let prop (sc: int) (sr: int) (dist: int) (w: int) =
                            ...
                            not (blocked got) && got.Col - start.Col <= distance && got.Col >= start.Col

                        Check.One(Config.QuickThrowOnFailure.WithMaxTest 500, prop)

                    test "distance ≤ 0 returns the start unchanged" {
                        Expect.equal
                            (Resolution.knockback (cell 5 5) (cell 1 0) 0 (fun _ -> false))
                            (cell 5 5)
                            "distance 0 ⇒ start"

                        Expect.equal
                            (Resolution.knockback (cell 5 5) (cell 1 0) -3 (fun _ -> false))
                            (cell 5 5)
                            "negative distance ⇒ start"
                    }

                    test "an immediately-blocked first step returns the start" {
                        Expect.equal
                            (Resolution.knockback (cell 0 0) (cell 1 0) 3 (fun c -> c.Col = 1))
                            (cell 0 0)
                            "next cell blocked ⇒ stay"
                    }
                ]
            ...
            testList
                "push (discrete grid, three relations and an observable stop reason)"
                [
                    ...
                    test "an unobstructed push takes every step and reports Completed" {
                        ...
                        Expect.equal
                            ...
                            "the cells entered, in order, excluding start"
                    }

                    test "Block stops the unit on the PREVIOUS cell, and names the obstacle" {
                        let wall (c: Cell) =
                            if c.Col = 2 then Resolution.Block else Resolution.Enter
                        ...
                        Expect.equal
                            r.Outcome
                            (Resolution.Blocked(cell 2 0))
                            "and the wall is named, so collision damage is attributable"
                    ...
                    test "Stop moves the unit ONTO the cell and stops it there — the relation `blocked` cannot express" {
                        let water (c: Cell) =
                            if c.Col = 2 then Resolution.Stop else Resolution.Enter
                    ...
                    test "Enter is passable-and-continue: a hazard is crossed, and every crossing is in Entered" {
                        ...
                        Expect.equal
                            (r.Entered |> List.filter isLava)
                            [ cell 1 0; cell 2 0 ]
                            "both lava cells were entered"

                        Expect.equal
                            r.Outcome
                            (Resolution.Stopped(cell 3 0))
                            "and it came to rest in the water beyond them"
                    }

                    test "an immediately-blocked first step keeps the start and enters nothing" {
                        let r =
                            Resolution.push (cell 0 0) east 3 (fun c ->
                                if c.Col = 1 then Resolution.Block else Resolution.Enter)
                    ...
                    <| fun () ->
                        let prop (sc: int) (sr: int) (dist: int) (w: int) (h: int) =
                            ...
                            let contiguous =
                                r.Entered
                                |> List.mapi (fun i c -> c = cell (start.Col + i + 1) start.Row)
                                |> List.forall id
            ...
            testList
                "knockback is EXACTLY the push shim (so the surface change is additive)"
                [
                    ...
                    <| fun () ->
                        let prop (sc: int) (sr: int) (stc: int) (str: int) (dist: int) (w: int) =
                            ...
                            let blocked (c: Cell) =
                                (abs c.Col + abs c.Row) % (1 + abs (w % 7)) = 0

                            let viaShim = Resolution.knockback start step distance blocked

                            let viaPush =
                                (Resolution.push start step distance (fun c ->
                                    if blocked c then Resolution.Block else Resolution.Enter))
                                    .Final
            ...
            testList
                "turn-based-tactics §4.6 falls out of a match on Outcome"
                [
                    ...
                    test "a Flying unit hovers over the water, because classify is the caller's" {
                        let flying (c: Cell) =
                            if c.Col >= 6 then Resolution.Block else Resolution.Enter
            ...
            testList
                "determinism golden (resolution transforms)"
                [
                    test "pushOut separates along +x by the depth" {
                        Expect.equal
                            (Resolution.pushOut (p 5. 5.) { Normal = p 1. 0.; Depth = 2. })
                            (p 3. 5.)
                            "pos - normal*depth"
                    }
                    ...
                    test "knockback stops in the last free cell before a wall" {
                        Expect.equal
                            (Resolution.knockback (cell 0 0) (cell 1 0) 3 (fun c -> c.Col = 2))
                            (cell 1 0)
                            "wall at col 2 ⇒ stop at (1,0)"
                    }
                    test "knockback with no blocker travels the full distance" {
                        Expect.equal
                            (Resolution.knockback (cell 0 0) (cell 1 0) 3 (fun _ -> false))
                            (cell 3 0)
                            "no wall ⇒ (3,0)"
                    }
