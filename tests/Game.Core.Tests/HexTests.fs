...
let private hexOf (q: int) (r: int) =
    Hex.create ((q % 20) - 10) ((r % 20) - 10)
...
let tests =
    testList
        "Game.Core Hex (019, FR-001..FR-008)"
        [
            ...
            testCase "create is always on-plane over random axials (FsCheck)"
            <| fun () ->
            ...
            testCase "ordinary-valid generator preserves construction and arithmetic laws (FsCheck)"
            <| fun () ->
                let prop =
                    Prop.forAll (Arb.fromGen ordinaryValidHexGen) (fun h ->
                        cubeSum h = 0L
            ...
            testCase "extreme-valid generator constructs exact boundary triples (FsCheck)"
            <| fun () ->
                let prop =
                    Prop.forAll (Arb.fromGen extremeValidCubeGen) (fun (q, r, s) ->
            ...
            testCase "malformed generator is rejected using a widened cube sum (FsCheck)"
            <| fun () ->
                let prop =
                    Prop.forAll (Arb.fromGen malformedCubeGen) (fun (q, r, s) ->
                        int64 q + int64 r + int64 s <> 0L && Hex.tryCreateCube q r s = None)
            ...
            test "the sealed public representation exposes read-only axes only" {
                ...
                Expect.isTrue
                    (publicProperties |> Array.forall (fun property -> not property.CanWrite))
                    "public axes are read-only"
            ...
            test "distance is the cube distance and is 1 to each neighbour (FR-002)" {
                ...
                Expect.isTrue
                    (Hex.neighbours h |> List.forall (fun n -> Hex.distance h n = 1))
                    "each neighbour is 1 away"
            ...
            testCase "distance equals max(|dq|,|dr|,|ds|) and neighbours are unit (FsCheck)"
            <| fun () ->
                let prop (aq: int) (ar: int) (bq: int) (br: int) =
                    ...
                    Hex.distance a b = byMax
                    && (Hex.neighbours a |> List.forall (fun n -> Hex.distance a n = 1))

                Check.One(Config.QuickThrowOnFailure.WithMaxTest 1000, prop)

            testCase "add/subtract/scale laws (FR-003, FsCheck)"
            <| fun () ->
                let prop (aq: int) (ar: int) (bq: int) (br: int) =
                    let a = hexOf aq ar
                    let b = hexOf bq br
            ...
            testCase "rotating six times is the identity and preserves origin distance (FR-004, FsCheck)"
            <| fun () ->
                let prop (q: int) (r: int) =
                    let h = hexOf q r
            ...
            test "range/ring/spiral cardinalities and relationships (FR-005)" {
                for n in 0..6 do
                    ...
                    Expect.equal
                        (List.length (Hex.spiral n))
                        (List.length (Hex.range n))
                        (sprintf "spiral %d = |range n|" n)
                    // spiral n is a permutation of range n
                    Expect.equal
                        (Hex.spiral n |> Set.ofList)
                        (Hex.range n |> Set.ofList)
                        (sprintf "spiral %d ≡ range n as sets" n)
            ...
            test "every ring hex is exactly n from the origin (FR-005)" {
                for n in 1..5 do
                    Expect.isTrue
                        (Hex.ring n |> List.forall (fun h -> Hex.distance Hex.origin h = n))
                        (sprintf "ring %d all at distance n" n)
            }
            ...
            testCase "lineDraw is contiguous and correct length over random pairs (FsCheck)"
            <| fun () ->
                let prop (aq: int) (ar: int) (bq: int) (br: int) =
                    let a = hexOf aq ar
                    let b = hexOf bq br
            ...
            testCase "offset/doubled converters round-trip (FR-007, FsCheck)"
            <| fun () ->
                let prop (q: int) (r: int) =
                    let h = hexOf q r
            ...
            test "Hex.astar/bfs find a shortest hop path on an open disc (FR-008)" {
                ...
                Expect.isTrue
                    (a |> Option.map (validHexPath walk start goal) |> Option.defaultValue false)
                    "astar path valid"

                Expect.isTrue
                    (b |> Option.map (validHexPath walk start goal) |> Option.defaultValue false)
                    "bfs path valid"

                Expect.equal
                    (a |> Option.map List.length)
                    (Some(Hex.distance start goal + 1))
                    "astar length = distance + 1 on open disc"

                Expect.equal
                    (b |> Option.map List.length)
                    (Some(Hex.distance start goal + 1))
                    "bfs length = distance + 1 on open disc"
            }
            ...
            test "Hex pathfinding degenerate/totality contract (FR-008)" {
                ...
                Expect.equal
                    (Hex.astar 100 walk (Hex.create 1 1) (Hex.create 1 1))
                    (Some [ Hex.create 1 1 ])
                    "start=goal ⇒ [start]"
            ...
            testCase "Hex astar and bfs agree on hop count + reachability, and are deterministic (FsCheck)"
            <| fun () ->
                let prop (aq: int) (ar: int) (bq: int) (br: int) (blockRaw: (int * int) list) =
                    let blocked = blockRaw |> List.map (fun (q, r) -> hexOf q r) |> Set.ofList

                    let walk h =
                        Hex.distance Hex.origin h <= 4 && not (Set.contains h blocked)
