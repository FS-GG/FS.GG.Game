...
let private smallGrid =
    [
        for c in -2 .. 2 do
            for r in -2 .. 2 -> cell c r
    ]

[<Tests>]
let tests =
    testList
        "Game.Core Los — grid line-of-sight (FS.GG.Game#32)"
        [
            ...
            test "lineOfSight defaults to Supercover — sight and shells never leak diagonally" {
                ...
                Expect.equal
                    (Los.lineOfSight isTransparent a b)
                    (Los.lineOfSightBy Supercover isTransparent a b)
                    "same as Supercover"
            ...
            test "both walks include both endpoints, `a` first, and connect correctly" {
                for a in smallGrid do
                    for b in smallGrid do
                        ...
                        Expect.equal
                            (List.length thin)
                            (max (abs (b.Col - a.Col)) (abs (b.Row - a.Row)) + 1)
                            $"thin length {a} -> {b}"
            }
            ...
            test "a single wall blocks exactly when it lies on the traced interior" {
                ...
                for mode in [ Thin; Supercover ] do
                    ...
                    for w in smallGrid do
                        let after =
                            Los.lineOfSightBy mode (transparentExcept (Set.ofList [ (w.Col, w.Row) ])) a b
            ...
            test "extreme coordinates do not wrap or throw (int64 delta promotion)" {
                ...
                Expect.equal
                    (Los.line (cell minC maxC) (cell minC maxC))
                    [ cell minC maxC ]
                    "degenerate at opposite extremes"

                Expect.isTrue
                    (Los.lineOfSight (fun _ -> true) (cell minC minC) (cell (minC + 3) (minC + 2)))
                    "los at the boundary"
            }
        ]
