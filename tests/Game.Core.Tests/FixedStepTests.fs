...
let tests =
    testList
        "Game.Core FixedStep drain (US3, FR-009/FR-010)"
        [
            ...
            test "a non-positive frame time contributes nothing new (a sub-interval accumulator is preserved)" {
                ...
                Expect.equal
                    (FixedStep.drain 0.0625 -3.0 0.03125)
                    (struct (0, 0.03125))
                    "negative dt ⇒ no steps, accumulator preserved"

                Expect.equal
                    (FixedStep.drain 0.0625 0.0 0.03125)
                    (struct (0, 0.03125))
                    "zero dt ⇒ no steps, accumulator preserved"
            }
            ...
            testList
                "totality — non-finite / nonsense inputs never throw, wedge, or go negative"
                [
                    ...
                    test "a NaN or infinite interval is a no-op with a finite accumulator" {
                        ...
                        Expect.equal
                            (FixedStep.drain infinity 0.1 0.1)
                            (struct (0, 0.1))
                            "infinite interval ⇒ struct(0, acc)"
                    }
                    ...
                    test "drainWith a degenerate clamp cannot produce negative steps or a NaN accumulator" {
                        Expect.equal
                            (FixedStep.drainWith -0.05 0.0625 1.0 0.0)
                            (struct (0, 0.0))
                            "negative clamp ⇒ no time admitted"

                        Expect.equal
                            (FixedStep.drainWith nan 0.0625 5.0 0.0)
                            (struct (0, 0.0))
                            "NaN clamp ⇒ no time admitted, no poison"
                    }
                    test "a remainder that rounds negative at an exact-multiple boundary is clamped to zero" {
                        ...
                        for (interval, frameTime, acc) in witnesses do
                            ...
                            Expect.isTrue
                                (rem >= 0.0)
                                $"drain {interval} {frameTime} {acc} ⇒ rem {rem} must not be negative"
                    ...
                    test "a pathologically tiny interval caps steps at Int32.MaxValue instead of wrapping negative" {
                        ...
                        Expect.isTrue
                            (rem > interval)
                            "documented exception: the uncounted steps overflow the interval bound"
                    }
                ]
