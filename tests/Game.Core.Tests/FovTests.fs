    ...
    seq {
        for row in -radius .. radius do
            for col in -radius .. radius do
                ...
                if sqDist c origin <= radius * radius then
                    c
    }
    |> Set.ofSeq
...
let tests =
    testList
        "Game.Core Fov — symmetric shadowcasting"
        [

            testList
                "totality and degenerate input"
                [
                    test "radius 0 sees exactly the origin" {
                        Expect.equal
                            (Fov.fov openField origin 0)
                            (Set.singleton origin)
                            "you always occupy your own cell"
                    }
                    ...
                    test "an opaque origin is still visible to itself" {
                        Expect.isTrue
                            (Fov.fov solidRock origin 3 |> Set.contains origin)
                            "you occupy the cell you stand in"
                    }
            ...
            testList
                "radius is an exact integer disc"
                [
                    test "an open field reveals precisely the Euclidean disc" {
                        for r in 0..6 do
                            Expect.equal
                                (Fov.fov openField origin r)
                                (disc r)
                                (sprintf "open field, radius %d ⇒ the disc" r)
                    }
                    test "a cell at exactly radius is visible, one beyond is not" {
                        ...
                        Expect.isTrue
                            (seen |> Set.contains { Col = 3; Row = 4 })
                            "3-4-5 triangle sits exactly on the rim"
                    }
                    testCase "field of view is always a subset of the disc (FsCheck ≥300 cases)"
                    <| fun () ->
                        let prop (blockedRaw: (int * int) list) (rRaw: int) =
                            ...
                            Fov.fov terrain o radius |> Set.forall (fun c -> sqDist c o <= radius * radius)

                        Check.One(Config.QuickThrowOnFailure.WithMaxTest 300, prop)
                ]

            testList
                "symmetry — the property that distinguishes shadowcasting from a ray per cell"
                [
                    testCase "transparent a sees transparent b ⇔ b sees a (FsCheck ≥200 random maps)"
                    <| fun () ->
            ...
            testList
                ...
                [
                    ...
                    test "a single pillar casts a clean shadow directly behind it" {
                        ...
                        for col in 3..6 do
                            Expect.isFalse
                                (seen |> Set.contains { Col = col; Row = 0 })
                                (sprintf "(%d,0) is directly behind the pillar" col)

                        // The shadow is a wedge, not a widening lattice of holes: cells off the axis are lit.
                        Expect.isTrue
                            (seen |> Set.contains { Col = 3; Row = 1 })
                            "off-axis behind the pillar stays visible"
                    ...
                    test "opacity is consulted, walkability is not — this module never sees a 'wall' flag" {
                        ...
                        Expect.isTrue
                            (seen |> Set.contains secretDoor)
                            "the door's near face is visible (expansive wall)"
            ...
            testList
                ...
                [
                    ...
                    testCase "no floating-point drift: repeated runs over random maps agree (FsCheck ≥300)"
                    <| fun () ->
                        let prop (blockedRaw: (int * int) list) (rRaw: int) =
                            let blocked =
                                blockedRaw
                                |> List.map (fun (c, r) -> ((abs c) % 9), ((abs r) % 9))
                                |> Set.ofList
