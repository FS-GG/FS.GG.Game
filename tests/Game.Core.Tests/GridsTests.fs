...
let private hEdge c r : Grids.Edge =
    {
        Col = c
        Row = r
        Orientation = Grids.Horizontal
    }

let private vEdge c r : Grids.Edge =
    {
        Col = c
        Row = r
...
let private spec size ox oy : Grids.GridSpec =
    {
        CellSize = size
        Origin = { X = ox; Y = oy }
    }
...
let tests =
    testList
        "Game.Core Grids (parts of a square grid)"
        [

            // --- adjacency: the fixed orderings are the contract ----------------------------------

            testList
                "adjacency orderings (documented, and therefore load-bearing)"
                [
            ...
            testList
                "adjacency is mutually consistent (FsCheck >=500)"
                [

                    testCase "every edge a cell reports, reports that cell back"
                    <| fun () ->
                    ...
                    testCase "every corner a cell reports, reports that cell back"
                    <| fun () ->
                        let prop (c: int) (r: int) =
                            let x = cell (smallIdx c) (smallIdx r)

                            Grids.cellCorners x
                            |> List.forall (fun v -> Grids.vertexCells v |> List.contains x)

                        Check.One(Config.QuickThrowOnFailure.WithMaxTest 500, prop)

                    testCase "every edge a vertex reports, reports that vertex back"
                    <| fun () ->
                        let prop (c: int) (r: int) =
                            let v = vtx (smallIdx c) (smallIdx r)

                            Grids.vertexEdges v
                            |> List.forall (fun e -> Grids.edgeVertices e |> List.contains v)

                        Check.One(Config.QuickThrowOnFailure.WithMaxTest 500, prop)

                    testCase "an edge separates exactly two distinct cells, and joins two distinct vertices"
                    <| fun () ->
                        let prop (c: int) (r: int) (vertical: bool) =
                            let e =
                                if vertical then
                                    vEdge (smallIdx c) (smallIdx r)
                                else
                                    hEdge (smallIdx c) (smallIdx r)
                    ...
                    testCase "the two cells an edge separates are orthogonal neighbours"
                    <| fun () ->
                        let prop (c: int) (r: int) (vertical: bool) =
                            let e =
                                if vertical then
            ...
            testList
                ...
                [
                    ...
                    test "edgeMidpoint bisects edgeSegment" {
                        Expect.equal
                            (Grids.edgeMidpoint (spec 16.0 0.0 0.0) (vEdge 2 3))
                            { X = 32.0; Y = 56.0 }
                            "midpoint"
                    }

                    testCase "edgeSegment is edgeVertices mapped through vertexPoint (FsCheck >=500)"
                    <| fun () ->
                    ...
                    test "cellAt is floor-based: a point on a boundary belongs to the cell right/below" {
                        ...
                        Expect.equal
                            (Grids.cellAt s { X = 31.999; Y = 48.0 })
                            (cell 1 3)
                            "a hair left is the cell before"

                        Expect.equal
                            (Grids.cellAt s { X = 47.999; Y = 63.999 })
                            (cell 2 3)
                            "a hair inside the BR corner"
                    }
            ...
            testList
                "cellAt inverts the pixel map (FsCheck >=500)"
                [

                    testCase "cellAt (min corner of cellRect spec c) = c, for every finite spec"
                    <| fun () ->
                    ...
                    testCase "cellAt (cellCenter spec c) = c, for every finite spec"
                    <| fun () ->
                        let prop (c: int) (r: int) (size: int) (ox: int) (oy: int) =
                            let s = spec (smallSize size) (smallOrigin ox) (smallOrigin oy)
                            let x = cell (smallIdx c) (smallIdx r)
                    ...
                    testCase "every point inside cellRect maps back to that cell (origin-offset too)"
                    <| fun () ->
            ...
            testList
                "totality: a degenerate GridSpec degrades, never throws and never leaks NaN"
                [
            ...
            test "identical input yields identical output (safe for a replayed step)" {
                ...
                let once =
                    Grids.cellEdges (cell 4 5), Grids.cellRect s (cell 4 5), Grids.cellAt s { X = 71.5; Y = 88.25 }
...
module TemplateTests =
    ...
    let private dist2 (a: Cell) (b: Cell) =
        int64 (a.Col - b.Col) * int64 (a.Col - b.Col)
        + int64 (a.Row - b.Row) * int64 (a.Row - b.Row)

    [<Tests>]
    let templateTests =
        testList
            "Game.Core Grids templates (024, FR-001..FR-004)"
            [
                ...
                test "disc is exactly the cells within the squared radius (FR-001)" {
                    ...
                    Expect.isTrue
                        (Grids.disc center 5 |> List.forall (fun c -> dist2 center c <= 25L))
                        "every disc cell within r²"
                }
                ...
                test "ring is the midpoint-circle outline at the given radius (FR-002)" {
                    ...
                    Expect.isTrue
                        (r
                         |> List.forall (fun c -> System.Math.Round(sqrt (float (dist2 center c))) = 5.0))
                        "every ring cell at rounded distance 5"
                ...
                test "ring cells lie on the disc boundary and ring scales (FR-002/FR-003)" {
                    ...
                    Expect.isTrue
                        (r
                         |> List.forall (fun c -> System.Math.Round(sqrt (float (dist2 center c))) = 500.0))
                        "every radius-500 ring cell at rounded distance 500"
                ...
                test "line templates reuse the shipped Los.line (FR-004)" {
                    ...
                    Expect.isTrue
                        (line
                         |> List.pairwise
                         |> List.forall (fun (x, y) -> abs (x.Col - y.Col) <= 1 && abs (x.Row - y.Row) <= 1))
                        "line is contiguous (each step adjacent)"
                }

                testCase "disc matches the brute-force squared-distance set over random radii (FsCheck)"
                <| fun () ->
