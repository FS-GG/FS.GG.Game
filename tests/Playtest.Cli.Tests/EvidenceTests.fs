...
let private trxXml (results: (string * string) list) (passed: int) (failed: int) : string =
    let body =
        results
        |> List.map (fun (name, outcome) ->
            sprintf "    <UnitTestResult testName=\"%s\" outcome=\"%s\" />" name outcome)
        |> String.concat "\n"
    ...
    testList
        ...
        [
            ...
            <| fun _ ->
                ...
                Expect.isTrue
                    (rows |> List.forall (fun r -> r.Result = "pass" && not r.Synthetic))
                    "every row satisfies"
            ...
            <| fun _ ->
                ...
                let proofs =
                    [ "GP-001", InputDriven; "GP-002", InputDriven; "GP-004", InputDriven ]
                    |> Map.ofList
                ...
                let resultOf id =
                    (rows |> List.find (fun r -> r.RequirementRefs = [ id ])).Result
                ...
                let satisfying =
                    ...
                    |> List.map (fun r -> r.RequirementRefs)
            ...
            <| fun _ ->
                ...
                let rendered =
                    Evidence.render "artifacts/testresults/playtest.trx" run (Evidence.rows run allInputDriven manifest)
            ...
            <| fun _ ->
                ...
                Expect.stringContains
                    rendered
                    "result: pass"
                    "the generated proof execution satisfies the production row"
